...
```bash
cd /opt/outception/deploy
# fetch a dump from R2 (or copy one down). Encrypted dumps (.enc) first:
#   BACKUP_PASSPHRASE="$(grep '^OUTCEPTION_BACKUP_PASSPHRASE=' .env.prod | cut -d= -f2-)" \
#     openssl enc -d -aes-256-cbc -pbkdf2 -iter 200000 \
#     -pass env:BACKUP_PASSPHRASE -in outception-<timestamp>.dump.enc -out outception-<timestamp>.dump
# Then restore. Single quotes matter: $POSTGRES_USER/$POSTGRES_DB must expand
# INSIDE the db container (the host shell has neither variable - the doubly
# quoted OUTCEPTION_* form expanded to empty strings and failed).
