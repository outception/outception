...
const OutceptionClientContext = createContext<{
  ...
  outception: createClient(API_URL, undefined, CLIENT_VERSION_HEADERS),
})
...
export function OutceptionClientProvider({ children }: PropsWithChildren) {
  ...
  const outception = useMemo(() => {
    const client = createClient(API_URL, session ?? '', CLIENT_VERSION_HEADERS)
    client.use(refreshMiddleware)
    return client
  }, [session])

  // The wrapper object must be memoized too: a fresh `{ outception }` per
  // render would invalidate every query hook in the app even when the client
  // itself hasn't changed.
  const value = useMemo(() => ({ outception }), [outception])

  return (
    <OutceptionClientContext.Provider value={value}>
      {children}
