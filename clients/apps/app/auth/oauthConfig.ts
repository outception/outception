// OAuth2 (PKCE) config for the mobile app. Everything is env-driven so wiring
// the app to a deployment is just setting env vars - no code edits.
//
// To get a CLIENT_ID:
//   1. Create an OAuth app at <web>/dashboard/account/developer
//   2. Choose "Public Client" and grant it the `scopes` below
//   3. Set the Redirect URI to "outception://oauth/callback"
//
// Then set, for your environment:
//   EXPO_PUBLIC_OUTCEPTION_SERVER_URL   - the API base (e.g. https://api.yourdomain)
//   EXPO_PUBLIC_OUTCEPTION_WEB_URL      - the web base (e.g. https://app.yourdomain)
//   EXPO_PUBLIC_OAUTH_CLIENT_ID    - the public client id from step 2

import { API_URL, WEB_URL } from '@/utils/env'

export const CLIENT_ID = process.env.EXPO_PUBLIC_OAUTH_CLIENT_ID ?? ''

export const discovery = {
  authorizationEndpoint: `${WEB_URL}/oauth2/authorize`,
  tokenEndpoint: `${API_URL}/v1/oauth2/token`,
  registrationEndpoint: `${API_URL}/v1/oauth2/register`,
  revocationEndpoint: `${API_URL}/v1/oauth2/revoke`,
}

// Reader-app scopes only. The merchant-dashboard set this used to request
// (organizations:write, members:write, org access-token minting) meant a
// stolen mobile token could do far more than read news - trimmed while
// accounts are off so re-enabling them is a deliberate decision, not a
// leftover.
export const scopes = ['openid', 'profile', 'email', 'user:read', 'user:write']
