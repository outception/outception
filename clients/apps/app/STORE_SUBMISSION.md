...
- Version `1.0.0`, set by `version` in `app.config.js` (the store-facing version —
  `package.json` tracks the npm package separately); production build numbers
  auto-increment (`eas.json` production profile)
- App icon / adaptive icon / splash assets present (`assets/images/`)
- No advertising: the app serves no ads and shows no ATT prompt (ads removed Aug 2026; native ad/ATT packages drop out in build 22)
...
- **Name:** Outception
- **Subtitle (iOS, ≤30):** A live deck of world news
- **Short description (Play, ≤80):** A live deck of headlines from 8,000+ sources, across every topic.
- **Full description:**
  > Outception is a live deck of the world's headlines. Follow 8,000+ news sources
  ...
  > • 10,000+ sources you can follow, filter by topic, and search
...
- **Does the app track?** No — the app serves no ads and collects no advertising identifiers.
- **Is data sold?** No.
- Third parties: analytics/diagnostics processors only (PostHog, Sentry).
