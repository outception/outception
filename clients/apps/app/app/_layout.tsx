// First: Intl polyfills must be in place before any module builds a formatter.
import '@/polyfills/intl'
import { SpectraBackground } from '@/components/Layout/SpectraBackground'
import { Box } from '@/components/Shared/Box'
import { DEFAULT_EDITION_ID } from '@/design-system/editions'
...
import { getLocaleSnapshot, whenLocaleHydrated } from '@/utils/locale'
import { getTranslations, loadTranslations } from '@outception-com/i18n'
...
import { warmCardPositions } from '@/components/News/useSwipeCards'
...
Sentry.init({
  ...
  // No IP/cookies/user on events - the app needs none of it for debugging.
  ...
  beforeBreadcrumb(breadcrumb) {
    // Query strings carry search terms (and card/lang params) - keep the path.
...
// Locale chunks are bundled modules (no network), so this is normally a few
// ms; the budget only guards a wedged AsyncStorage read.
const I18N_READY_BUDGET_MS = 1_500
// Start the stored-language read at import, alongside the font load, rather
// than after the first render has committed and its effect runs.
void whenLocaleHydrated()
...
export default Sentry.wrap(function RootLayout() {
  ...
  const themeSet = editionThemes[edition] ?? editionThemes[DEFAULT_EDITION_ID]
  ...
  // sources list, card set, and consent calls are competing for the radio.
  ...
  useEffect(() => {
    // Pull saved card set positions into memory before the wall mounts, so the card set
    // can seed its first render from them instead of painting card 1 and then
    // jumping to the saved one.
    warmCardPositions()
  }, [])

  // Hold the splash until the reader's language is known (their picker choice
  // hydrates from storage) and its strings are loaded, so the first frame
  // paints translated - the web awaits the same before rendering. Non-English
  // strings evaluate lazily, so without this the wall opened in English and
  // switched a beat later. Bounded: a stalled storage read must not keep the
  // splash up, English is the fallback either way.
  ...
  useEffect(() => {
    ...
    const budget = setTimeout(ready, I18N_READY_BUDGET_MS)
    void whenLocaleHydrated()
      .then(() => loadTranslations(getLocaleSnapshot()))
      .catch(() => {})
      .finally(ready)
              ...
              <OutceptionClientProvider>
                <OutceptionQueryClientProvider>
                  ...
                  >
                    ...
                    />
                    {/* App-wide, and deliberately not per-screen: the host
                        renders nothing until someone raises a sheet, and
                        mounting it on one route means a menu raised from any
                        OTHER route would silently show nothing at all. */}
                    <ActionSheet />
                  </Box>
                </OutceptionQueryClientProvider>
              </OutceptionClientProvider>
...
export function ErrorBoundary({ error, retry }: ErrorBoundaryProps) {
  ...
  // The message goes to Sentry, not the screen: it can carry URLs or payload
  // fragments the reader has no use for.
  useEffect(() => {
