 ...
 * this cadence (mirrors web). Measured on the live API: a cold card serves
 * originals at the 1 s soft wait and the translations land 1-2 s after
 * that, so a 4 s poll left readers looking at English for most of 6 s. Polls
 * are cache-first on the server, so the faster cadence costs nothing upstream. */
...
/** How many fast translation polls a card may fire before falling back to the
 * normal cadence (~24 s): a server stuck reporting `translationsPending` must
 * not keep a card refetching fast for the whole session. */
  ...
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return translationPolls.current.count > TRANSLATION_POLL_LIMIT
        ? 60_000
        : translationPollDelayMs(translationPolls.current.count)
    },
