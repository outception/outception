 ...
 * the shared guts of both the tap-to-spin mark and the auto-looping loader. */
const SpinSprite = ({
  size,
  frame,
  ramp,
...
const useBrandRamp = (): readonly string[] => {
  ...
  return BRAND_RAMPS[edition] ?? BRAND_RAMPS.phosphor
}

/**
 * The same spinning-top animation, but playing on its own in an endless loop —
 * no tap needed. Used as the loading mark when the app opens, so the splash
 * logo appears to come alive and keep turning until the wall is ready.
 */
export const AutoSpinningLogo = ({ size }: { size: number }) => {
  ...
  useEffect(() => {
    ...
    frame.value = withRepeat(
      withTiming(SPIN_FRAMES, { duration: SPIN_MS, easing: Easing.linear }),
      -1,
    )
    return () => cancelAnimation(frame)
  }, [frame])

  return <SpinSprite size={size} frame={frame} ramp={ramp} />
}
