...
export const BRAND_RAMPS: Record<string, readonly string[]> = {
  // Mirrors the web's midnight edition --color-brand-100…900 (globals.css) -
  // the DEFAULT edition. It took the mark's red over when the grey page
  // replaced the old ruby edition; leaving the blue-grey ramp here rendered a
  // blue gem beside red accents on the very same screen.
  midnight: [
    '#ffd7da',
    '#ffadb4',
    '#ff8290',
...
const LogoIcon = ({ size = 24 }: { size?: number }) => {
  ...
  const r = BRAND_RAMPS[edition] ?? BRAND_RAMPS[DEFAULT_EDITION_ID]
  return (
