...
const posKey = (key: string) => `news.cardPos.${key}`

// Saved positions, warmed once at app start so the card set can seed its initial
...
// that, so without this pre-warm the card set painted card 1 and then visibly
...
/** Load every saved card set position into `positionCache`. Safe to call repeatedly. */
export const warmCardPositions = (): void => {
  if (positionsLoaded || positionsInFlight) return
  positionsInFlight = true
  void AsyncStorage.getAllKeys()
    .then((keys) => {
      const ours = keys.filter((k) => k.startsWith('news.cardPos.'))
...
/** Where the card set should sit after the item list changes: jump to a newly
...
/**
 * Owns the card set's position: which card is on top and the prev/next moves,
 * anchored by SOURCE ID (not index) so adding/reordering sources doesn't yank
 * the card set around. With a `storageKey` the active card is persisted per key
...
export const useSwipeCards = (
  ...
  // this its stale saved id resolves a few ms later and snaps the card set back off
  // the card the reader just followed.
  const focusApplied = useRef(false)

  // Restore the saved card position once per storageKey (the card set remounts per
  ...
  // Wrap around so the card set loops endlessly: stepping past the last card lands
  // back on the first and stepping back from the first lands on the last. A
  // single-card stack just stays put.
  ...
  // Module-scoped, not a ref: the card set can remount (e.g. the empty card set's
  // "Browse sources" state swaps CardsBody), and a per-instance ref would
  // re-initialise to the already-bumped seq on return and swallow the jump.
  // The Sources dialog now floats OVER a still-mounted card set, but remounts
