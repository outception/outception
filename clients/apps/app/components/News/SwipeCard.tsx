...
// Swipe physics (mirroring the web SwipeCard). Smaller peek than web
...
// Memoized so parent card set re-renders (follow toggles, failed-source updates)
// don't re-create the pan gesture + worklets and re-render the card subtree.
// Effective only while every prop is stable - NewsCards passes a ref-backed
...
export const SwipeCard = memo(function SwipeCard({
  ...
  rtl?: boolean
  /** Strip left free below the card (the card set's ad banner sits there). */
  bottomInset?: number
  ...
  const settle = (move: CardsMove | null) => {
    if (move) {
  ...
  // dx; the card set applies its own direction, RTL and bounds rules.
  useEffect(() => {
    if (!isTop) return
  ...
  // While an inline summary is open on the top card the card set goes
  ...
  const pan = Gesture.Pan()
    ...
    // rotation drags) - the card set swipes only from the header strip, exactly
  ...
  if (summaryOpen) {
    ...
    // and a horizontal stroke now belongs to the card set.
  }
