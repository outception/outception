...
import { useDefaultDeck, useWallSourceMetas } from '@/hooks/outception/news'
...
export const NewsFeed = ({
  ...
  // Your deck: the followed set wins; an empty followed set falls back to the
  // seeded default deck.
  ...
  // A shared card link surfaces its source at the FRONT of the wall even if the
  // recipient doesn't follow it, so they land on exactly what was shared.
  ...
  // The deck paints from metadata for just these ids — never the whole roster,
  // which only the source browser needs (it loads that when it opens).
  ...
  const visible = useMemo<NewsSourceMeta[]>(() => {
    ...
    return deck
  }, [metas, deckIds, hidden, failed, sharedCardId])

  // A fresh reader's deck is seeded from the default-deck query; until that
