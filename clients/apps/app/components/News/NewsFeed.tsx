...
export const NewsFeed = ({
  ...
  // (see focusedSources.toggleFocus) instead of collapsing the card set to one card.
  useEffect(() => {
    setSeedCards(defaultCardIds ?? [])
  }, [defaultCardIds])

  // Your stack: the followed set wins; an empty followed set falls back to the
  // seeded default card set.
  ...
  // The card set paints from metadata for just these ids - never the whole roster,
  ...
  const visible = useMemo<NewsSourceMeta[]>(() => {
    ...
    return cards
  }, [metas, cardIds, hidden, failed, sharedCardId])
