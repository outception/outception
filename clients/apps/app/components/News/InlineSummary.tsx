...
// The gesture-handler ScrollView + a Gesture.Native() wrapper: the summary's
// scroll must be representable AS A GESTURE OBJECT so the card set's pan can
// block it while a summary is open (vertical drags scroll, horizontal ones
// swipe the card). Blocking via component refs - the first attempt - is the
// documented-unreliable path on iOS.
import {
  Gesture,
  GestureDetector,
  ScrollView,
...
// A publisher teaser can land in its ORIGINAL language while its translation
// runs in the background (the server says so with translationsPending). Poll
// a few times for the translated line and swap it in place, instead of
// leaving the panel in the wrong language until the next tap. Bounded, and
// cache-first on the server, so a stuck translation costs a handful of
// cheap requests at most. Same cadence as the cards' pending-translation
// poll (see hooks/outception/news.ts): the batch lands 1-2 s after the
// server's soft wait gives up, so a slower poll only shows the wrong
// language for longer.
const TEASER_POLL_MS = 1_500
const TEASER_POLL_LIMIT = 4

/** Module-level so the gesture worklet can runOnJS a stable function. */
const dispatchSummarySwipe = (dx: number) => {
  getSummarySwipeHandler()?.(dx)
}
...
export const InlineSummary = ({
  ...
  // The summary owns its horizontal pan and composes it EXCLUSIVELY with its
  // own scroll - all in one component, the only gesture arbitration iOS
  // honors reliably (three cross-component attempts failed on device). A
  // finished horizontal stroke is handed to the card set via the store.
  const [swipeGesture] = useState(() => {
    ...
    const pan = Gesture.Pan()
      ...
      .onEnd((e) => {
        ...
        if (Math.abs(e.translationX) > 40 || Math.abs(e.velocityX) > 500) {
          runOnJS(dispatchSummarySwipe)(e.translationX)
        }
      })
    return Gesture.Exclusive(pan, scrollNative)
  })
  ...
  useEffect(() => {
    ...
    let teaserPoll: ReturnType<typeof setTimeout> | undefined
    const openArticle = () => {
    ...
    const load = () => {
      ...
      const pollTranslation = (attempt: number) => {
        teaserPoll = setTimeout(() => {
          if (runId.current !== run) return
          api
            .summary(url, locale)
            .then((d) => {
              if (runId.current !== run) return
              if (d.translationsPending) {
                if (attempt < TEASER_POLL_LIMIT) pollTranslation(attempt + 1)
                return
              }
              // The text is already on screen: swap the line, no re-typing.
              setState({
                kind: 'done',
                text: d.summary,
                teaser: d.kind === 'teaser',
                source: sourceName,
              })
            })
            .catch(() => {
              // The original line stays; the next tap serves the translation.
            })
        }, TEASER_POLL_MS)
      }
      ...
      api
        ...
        .then((d) => {
          ...
          if (d.translationsPending) pollTranslation(1)
        })
        .catch(() => {
          if (deadline) clearTimeout(deadline)
    ...
    return () => {
      ...
      runId.current++
      if (deadline) clearTimeout(deadline)
      if (teaserPoll) clearTimeout(teaserPoll)
