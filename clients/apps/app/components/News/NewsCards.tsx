...
// hair of air) - the cards' bottom inset, so the banner reads as the card set's
...
/**
 * Swipeable card stack: the centred card follows the finger and rotates, then
 ...
 * mirror this. Ports the web NewsCards to React Native.
 */
  ...
  return (
    <Box flex={1} gap="spacing-16" paddingHorizontal="spacing-4">
      ...
      <Box flex={1} style={{ position: 'relative' }}>
        {/* No MaskedView here: wrapping the card set in a mask layer put a view
      ...
      >
        <Touchable
          onPress={cards.goPrev}
          disabled={!cards.canPrev}
          accessibilityLabel={t('news.cards.previous')}
        >
          <Box
            padding="spacing-6"
            ...
            style={{ opacity: cards.canPrev ? 1 : 0.4 }}
          >
            <MaterialIcons
              name={prevIcon}
              size={16}
        ...
        <Box
          ...
          alignItems="center"
          gap="spacing-6"
          accessibilityLabel={`${cards.position} / ${cards.total}`}
        >
          {pipWindow(cards.position, cards.total).map((i) => (
            <Pip
              key={i}
              active={i === cards.position}
              color={theme.colors.pageEndText}
            />
          ))}
        </Box>
        ...
        <Touchable
          onPress={cards.goNext}
          disabled={!cards.canNext}
          accessibilityLabel={t('news.cards.next')}
        >
          <Box
            padding="spacing-6"
            borderRadius="border-radius-12"
            borderWidth={1}
            borderColor="pageEndBorder"
            style={{ opacity: cards.canNext ? 1 : 0.4 }}
          >
            <MaterialIcons
              name={nextIcon}
              size={16}
