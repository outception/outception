import { Box } from '@/components/Shared/Box'
import { Image } from '@/components/Shared/Image/Image'
import { Text } from '@/components/Shared/Text'
import { Touchable } from '@/components/Shared/Touchable'
import { useTheme } from '@/design-system/useTheme'
import { useT } from '@/providers/LocaleProvider'
import {
  isSummarizable,
  openExternalUrl,
  timeAgo,
  type NewsItem,
} from '@/utils/news'
import { InlineSummary } from './InlineSummary'
import { KICKER_STYLE } from './newsStyles'
import type { useClipPartialRows } from './useClipPartialRows'

type Clip = ReturnType<typeof useClipPartialRows>

type SummaryProps = {
  sourceName?: string
  expandedSummary: string | null
  onToggleSummary: (key: string | null) => void
}

/** Plain taps open the inline AI summary (like the web); links the server
 * can never summarize (videos) go straight to the article instead. */
const pressAction = (
  item: NewsItem,
  key: string,
  expanded: string | null,
  toggle: (key: string | null) => void,
) => {
  if (!isSummarizable(item.url)) {
    openExternalUrl(item.url)
    return
  }
  toggle(expanded === key ? null : key)
}

const isHttpUrl = (url: string | null | undefined): url is string =>
  !!url && /^https?:\/\/\S+$/i.test(url)

/** A short source whose whole feed fits on the card gets a caught-up line
 * under its last row (mirrors the web wall's end-of-feed note) - only when
 * every row actually rendered, and never on a longer (9+) list where the
 * line would just be sliced by the card clip. */
const CaughtUpNote = ({ items, clip }: { items: NewsItem[]; clip: Clip }) => {
  const t = useT()
  if (
    items.length === 0 ||
    items.length > 8 ||
    clip.visibleCount < items.length
  )
    return null
  return (
    <Text variant="caption" color="subtext" numberOfLines={2}>
      {t('news.caughtUp.body')}
    </Text>
  )
}

/** The timeline timestamp for a headline: the publish time, falling through to
 * the feed's `extra.date` (number → relative, string → shown verbatim). Mirrors
 * the web timeline's timestamp fallback so realtime sources that supply their
 * time in `extra.date` still show one. */
const timelineStamp = (
  item: NewsItem,
  now: number,
  locale: string,
): string | null => {
  if (typeof item.pubDate === 'number')
    return timeAgo(item.pubDate, now, locale)
  if (typeof item.extra?.date === 'number') {
    return timeAgo(item.extra.date, now, locale)
  }
  if (typeof item.extra?.date === 'string') return item.extra.date
  return null
}

/** Source-supplied trailing metadata: a short string (e.g. Hacker News points /
 * Reddit comment counts) or a small icon. The icon URL comes from an untrusted
 * feed, so it's restricted to http(s) before use. Mirrors the web ExtraInfo. */
const ExtraInfo = ({ item }: { item: NewsItem }) => {
  const theme = useTheme()
  if (item.extra?.info) {
    return (
      <Text variant="caption" color="subtext">
        {item.extra.info}
      </Text>
    )
  }
  const icon = item.extra?.icon
  if (isHttpUrl(icon)) {
    const size = theme.dimension['dimension-16']
    return <Image source={icon} style={{ width: size, height: size }} />
  }
  return null
}

/** The trailing `info` string appended inline after a headline (the common
 * aggregator case), falling back to the icon form like the web's ExtraInfo -
 * icon-only sources otherwise show nothing where web shows the favicon. */
const InlineInfo = ({ item }: { item: NewsItem }) =>
  item.extra?.info ? (
    <Text variant="caption" color="subtext">
      {` ${item.extra.info}`}
    </Text>
  ) : isHttpUrl(item.extra?.icon) ? (
    <ExtraInfo item={item} />
  ) : null

/** Ranked list for "hottest" sources - the top story runs large, the rest are
 * numbered rows separated by hairlines. Mirrors the web NewsListHot. */
export const NewsListHot = ({
  items,
  clip,
  onItemLongPress,
  sourceName,
  expandedSummary,
  onToggleSummary,
}: {
  items: NewsItem[]
  clip: Clip
  onItemLongPress?: (item: NewsItem) => void
} & SummaryProps) => {
  const theme = useTheme()
  const expandedIndex = expandedSummary
    ? items.findIndex((it) => it.id === expandedSummary)
    : -1
  return (
    <Box gap="spacing-4">
      {(expandedIndex >= 0
        ? items.slice(expandedIndex, expandedIndex + 12)
        : items.slice(0, clip.visibleCount)
      ).map((item, j) => {
        const i = expandedIndex >= 0 ? expandedIndex + j : j
        return (
          // The summary panel is a SIBLING of the headline's touch target (as
          // on web): a tap inside the summary must not collapse it. The keyed
          // wrapper carries the row measurement so the pinned row's height
          // includes the open panel.
          <Box
            key={`${item.id}-${i}`}
            onLayout={expandedIndex >= 0 ? undefined : clip.onRowLayout(i)}
          >
            <Touchable
              onPress={() =>
                pressAction(item, item.id, expandedSummary, onToggleSummary)
              }
              onLongPress={
                onItemLongPress ? () => onItemLongPress(item) : undefined
              }
            >
              {i === 0 ? (
                <Box paddingBottom="spacing-8">
                  <Text variant="leadSerif" numberOfLines={3}>
                    {item.title}
                    <InlineInfo item={item} />
                  </Text>
                </Box>
              ) : (
                <Box
                  flexDirection="row"
                  gap="spacing-8"
                  alignItems="flex-start"
                  paddingVertical="spacing-4"
                  style={{
                    borderTopWidth: 1,
                    // Web's row rules are ink at 7-8% over the sheet - the
                    // opaque border token is the header rule's weight and read
                    // as a ruled grid here.
                    borderTopColor: `${theme.colors['foreground-regular']}14`,
                  }}
                >
                  <Text
                    variant="caption"
                    color="subtext"
                    style={{
                      minWidth: theme.dimension['dimension-20'],
                      textAlign: 'center',
                      paddingTop: theme.spacing['spacing-2'],
                    }}
                  >
                    {i + 1}
                  </Text>
                  <Box flex={1}>
                    <Text variant="bodySerif" numberOfLines={2}>
                      {item.title}
                      <InlineInfo item={item} />
                    </Text>
                  </Box>
                </Box>
              )}
            </Touchable>
            {expandedSummary === item.id && item.url ? (
              <InlineSummary
                url={item.url}
                sourceName={sourceName}
                onClose={() => onToggleSummary(null)}
              />
            ) : null}
          </Box>
        )
      })}
      {expandedIndex >= 0 ? null : <CaughtUpNote items={items} clip={clip} />}
    </Box>
  )
}

/** Timeline list for "realtime" sources - each row a timestamp kicker above the
 * headline, with air between rows. Mirrors the web NewsListTimeline. */
export const NewsListTimeline = ({
  items,
  now,
  clip,
  onItemLongPress,
  sourceName,
  expandedSummary,
  onToggleSummary,
}: {
  items: NewsItem[]
  now: number
  clip: Clip
  onItemLongPress?: (item: NewsItem) => void
} & SummaryProps) => {
  const expandedIndex = expandedSummary
    ? items.findIndex((it) => it.id === expandedSummary)
    : -1
  return (
    <Box gap="spacing-12">
      {(expandedIndex >= 0
        ? items.slice(expandedIndex, expandedIndex + 12)
        : items.slice(0, clip.visibleCount)
      ).map((item, j) => {
        const i = expandedIndex >= 0 ? expandedIndex + j : j
        const stamp = timelineStamp(item, now, 'en')
        const hasExtra = !!item.extra?.info || isHttpUrl(item.extra?.icon)
        return (
          // Summary as a sibling of the touch target - see NewsListHot.
          <Box
            key={`${item.id}-${i}`}
            onLayout={expandedIndex >= 0 ? undefined : clip.onRowLayout(i)}
          >
            <Touchable
              onPress={() =>
                pressAction(item, item.id, expandedSummary, onToggleSummary)
              }
              onLongPress={
                onItemLongPress ? () => onItemLongPress(item) : undefined
              }
            >
              <Box gap="spacing-4">
                {stamp || hasExtra ? (
                  <Box flexDirection="row" alignItems="center" gap="spacing-4">
                    {stamp ? (
                      <Text
                        variant="caption"
                        color="subtext"
                        style={KICKER_STYLE}
                      >
                        {stamp}
                      </Text>
                    ) : null}
                    <ExtraInfo item={item} />
                  </Box>
                ) : null}
                {/* Web's lead row is heading-xxs (18/500 serif) - leadSerif is
                    the app's purpose-built match; `title` ran a step heavier. */}
                <Text
                  variant={i === 0 ? 'leadSerif' : 'bodySerif'}
                  numberOfLines={i === 0 ? 3 : 2}
                >
                  {item.title}
                </Text>
              </Box>
            </Touchable>
            {expandedSummary === item.id && item.url ? (
              <InlineSummary
                url={item.url}
                sourceName={sourceName}
                onClose={() => onToggleSummary(null)}
              />
            ) : null}
          </Box>
        )
      })}
      {expandedIndex >= 0 ? null : <CaughtUpNote items={items} clip={clip} />}
    </Box>
  )
}
