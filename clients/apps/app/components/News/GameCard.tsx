...
/** How much of the card must survive the phone keyboard: below this the board
 * is unusable, so the lift stops rather than collapsing the card. */
...
export const GameCard = memo(function GameCard({
  ...
  // How far the phone keyboard pushes the card up. The grid games focus a
  // hidden input, so without this the keyboard covers the board.
  ...
  // Phone keyboard: the deck's cards are absolutely positioned (see
  // SwipeDeckCard), so rather than doing unreliable height math we lift the
  // card off the bottom edge by the keyboard's height — the flex board above
  // shrinks with it and rides clear of the keys. Clamped so the card never
  // shrinks past MIN_CARD_HEIGHT, and only the top card listens.
  //
  // iOS only: Android runs the Expo default softwareKeyboardLayoutMode
  // "resize", so the window has ALREADY shrunk by the keyboard's height by
  // the time the event lands — lifting there would move the card twice.
  useEffect(() => {
    if (!elevated || Platform.OS !== 'ios') return
    const onShow = Keyboard.addListener('keyboardDidShow', (e) => {
      const height = e.endCoordinates?.height ?? 0
      const room = Math.max(0, naturalHeight.current - MIN_CARD_HEIGHT)
      setKeyboardLift(Math.min(height, room))
    })
  ...
  return (
    <Box
      ...
      borderWidth={1}
      borderColor="border"
      // Measured while the keyboard is DOWN — that's the card's natural height,
      // the budget the lift is clamped against (measuring after the margin
      // lands would chase its own tail).
      onLayout={(e) => {
        if (keyboardLift === 0) {
          naturalHeight.current = e.nativeEvent.layout.height
        }
      }}
      style={[
        elevated
