 ...
 * feed drops it.
 *
 * Memoized: the card set re-renders on every follow toggle / failed-source update
...
export const NewsSourceCard = memo(function NewsSourceCard({
  ...
  const expandedIndex = expandedSummary
    ...
    : -1
  // Publish the open summary so the card set's pan gesture stands down - a
  // vertical drag must scroll the summary, not throw the card (see
  // SwipeCard). Only the elevated card ever touches the store - a peek
  ...
  return (
    <Box
      ...
      // there. It's safe alongside the card set's zIndex because it runs the same
        ...
        >
          ...
          <Box gap="spacing-2" flexShrink={1}>
            ...
            <Text variant="caption" color="subtext" style={KICKER_STYLE}>
              {data?.updatedTime
                ? `${t('news.card.updated')} ${timeAgo(data.updatedTime, now, 'en')}`
                : isError
                  ? t('news.card.failed')
                  : t('news.card.loading')}
            </Text>
