 ...
 *
 * A FRESH visitor's empty followed set falls back to the seeded default deck
 * (see NewsFeed); a deck the reader explicitly emptied ("Deselect all") stays
 * empty - `deckTouched` tells the two apart.
 */
...
let hydrated = false
let inFlight = false
// True once a deck has ever been written on this device ('[]' counts) -
...
const hydrate = () => {
  ...
  void AsyncStorage.getItem(FOCUSED_KEY)
    .then((rawFocused) => {
      if (rawFocused !== null) deckTouched = true
...
/** True when the reader explicitly emptied their deck ("Deselect all") - the
 * wall must show the empty state instead of re-seeding the default deck. */
export const getDeckClearedSnapshot = (): boolean =>
  deckTouched && focused.length === 0
...
export const toggleFocus = (id: string): void => {
  ...
  const base =
    adding && focused.length === 0 && !deckTouched && seedDeck.length > 0
...
export const followAll = (ids: string[]): void => {
  ...
  const base =
    focused.length === 0 && !deckTouched && seedDeck.length > 0
      ? seedDeck.filter((x) => !getHiddenSnapshot().includes(x))
      : focused
 ...
 * followed set. Persisting the emptied list marks the deck as explicitly
 * cleared (see getDeckClearedSnapshot) so the wall shows the empty state
 * instead of re-seeding the default deck. */
