...
/** A link that opens the web wall on this exact source (the web landing page
 * reads `?card`). Shared from the mobile card so recipients get the same rich
 * preview as a web share. */
export const shareCardUrl = (sourceId: string): string =>
  `${WEB_URL}/?card=${encodeURIComponent(sourceId)}`
...
const relativeFormatter = (
  ...
  if (!fmt) {
    // numeric 'auto' matches the web wording: "now" for 0, and
