...
const relativeFormatter = (
  ...
  if (!fmt) {
    ...
    // Filipino is `tl` in our locale list but `fil` in CLDR, which is the
    // only tag the relative-time data ships under (see polyfills/intl).
    fmt = new Intl.RelativeTimeFormat(locale === 'tl' ? 'fil' : locale, {
      style: 'narrow',
      numeric: 'auto',
    })
