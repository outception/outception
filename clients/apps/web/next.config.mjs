  ...
  async rewrites() {
    ...
    return [
      ...(PREVIEW_BUILD && apiUrl
        ...
        : []),
      // /ingest/* is handled by src/app/ingest/[...path]/route.ts, which
      // strips cookies before forwarding: a plain rewrite proxied the whole
      // request - session cookie included - to PostHog's servers.
    ]
  },
  ...
  async headers() {
    ...
    return [
      ...
      },
      {
        ...
        // keeps remounts snappy while fixes still reach devices in about a
        // minute. The day-long stale-while-revalidate this used to carry
        // meant a phone could keep replaying yesterday's page while every
        // "is it fixed yet" test silently ran one deploy behind.
      ...
      {
        // 978 source icons, served straight out of public/ with Next's default
        // `max-age=0` (and therefore bypassed by the CDN). They're content-
        // stable: a day fresh plus a week of stale-while-revalidate.
        source: '/news-icons/:path*',
        headers: [
          {
            key: 'Cache-Control',
      ...
      {
        // Variable font files are versionless and preloaded on every page -
        // four hours meant repeat visits kept revalidating them. Not
        // `immutable`: the filenames never change, so a replaced font must
        // still reach returning visitors within the stale window.
        source: '/fonts/:path*',
        headers: [
          {
            key: 'Cache-Control',
