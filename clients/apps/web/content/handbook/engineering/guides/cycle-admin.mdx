---
title: "Outception Cycle"
description: "How to configure Outception Cycle, the Slack bot that announces PRs and rotates the primary reviewer"
---

Outception Cycle is the small Slack bot that posts PR threads in `#pull-requests` and picks a primary reviewer whenever a team is requested for review. It runs on Vercel at [outception-cycle.vercel.app](https://outception-cycle.vercel.app), with a tiny admin UI for the bits we need to tweak day to day.

This guide covers what the admin panel does and the most common tasks you will run from it. The implementation lives in [`outception/cycle`](https://github.com/outception/cycle).
...
Go to [outception-cycle.vercel.app/admin](https://outception-cycle.vercel.app/admin).

### Sign in with GitHub

Click **Sign in with GitHub**. You must be a member of `@outception` — Cycle calls the GitHub API with your token to verify org membership before issuing a session. Anyone in the org can sign in and edit configuration; there are no further role checks.
...
A newline-separated list of `owner/repo` slugs that Cycle should listen to (e.g. `outception/outception`). Cycle ignores webhooks for any repo not in this list, so adding a new repo here is the switch that turns Cycle on for it.

<Note>
  Webhooks are configured at the GitHub App level (separately from this panel). If you add a
  brand-new repo and Cycle is not posting anything, double-check that the **Outception Cycle** GitHub App
...
- **GitHub** is the user's GitHub login (e.g. `octocat`).
...
1. Make sure the **Outception Cycle** GitHub App is installed on the repo (org admins can do this).
...
- Source: [`outception/cycle`](https://github.com/outception/cycle)
- Admin URL: https://outception-cycle.vercel.app/admin
- Storage: Upstash Redis (configured via Vercel env vars)
- Auth: GitHub OAuth, restricted to `@outception` org members, signed-cookie sessions (7-day TTL)
