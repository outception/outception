...
- Double-check identifiers before executing queries
...
On BetterStack: `https://uptime.betterstack.com/team/{TEAM_ID}/oncalls/{SCHEDULE_ID}` (see the team's internal docs for the IDs).

This is also where you edit/update the schedule.

You can find people on the rota and their phone numbers in the BetterStack
team-members settings (`https://betterstack.com/settings/teams/{TEAM_ID}/team-members`).
...
- BetterStack itself (uptime monitoring of the API and the web app)
- Sentry alerts
...
- For major incidents – i.e. all of Outception is down or we’ve had more than 5 minutes of downtime then always escalate to other team members – don’t try to be the hero.

**How?**

- In BetterStack use the “Escalate” button to escalate to the secondary on-call engineer. If that doesn’t work, use the BetterStack team page and call the phone numbers listed there. Note that BetterStack’s own escalation mechanism is preferred as those calls and notifications are more likely to come through.

## Support

- The week you’re on call, you’re also the go-to person for the support team to reach out to for deeper, more technical questions or things that can only be solved by a dev.
