...
- [ ] Document the incident for handover
- [ ] Notify the team
...
- **Transaction open across external I/O** — e.g. a worker that queries the DB and then makes a slow HTTP call (Expo push, the payment gateway, etc.) while still inside the same database transaction. The transaction never commits until the HTTP call returns.
