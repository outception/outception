...
description: "How to use task debouncing to avoid duplicate task execution in Outception's background worker system."
---

Task debouncing is a mechanism used in Outception's background worker system to prevent duplicate execution of tasks that are triggered multiple times in a short amount of time. This is particularly useful for operations that might be triggered by multiple events or user actions.
...
```python
from outception.worker import actor
...
- `outception_task_debounced_total`: Counter of tasks that were skipped due to debouncing
- `outception_task_debounce_delay_seconds`: Histogram of delays between first enqueue and execution
