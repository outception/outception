---
title: "Metadata"
description: "A shared mechanism letting users attach arbitrary key-value data to Outception models."
---

Metadata is a shared mechanism that lets Outception users attach arbitrary key-value pairs to supported resources (any user-facing API resource). It is entirely for the benefit of integrators — Outception itself does not use these values — and is exposed as a plain `metadata` field in every API request and response.
...
- Users are likely to need to cross-reference Outception records with their own systems (e.g. storing an internal record ID, a feature flag, or a campaign tag).

Do **not** use `user_metadata` to store Outception-internal data; use dedicated columns for that.
...
```python
from outception.kit.metadata import MetadataMixin

class Promotion(MetadataMixin, RecordModel):
    ...
```
...
```python
from outception.kit.metadata import MetadataInputMixin, MetadataOutputMixin

class PromotionCreate(MetadataInputMixin, Schema):
    ...

class PromotionUpdate(MetadataInputMixin, Schema):
    ...

class Promotion(MetadataOutputMixin, TimestampedSchema, IDSchema):
    ...
```
...
```python
# endpoints.py
from outception.kit.metadata import MetadataQuery, get_metadata_query_openapi_schema
...
async def list(
    ...
    items, count = await promotion_service.list(session, ..., metadata=metadata)
```

```python
# service.py
from outception.kit.metadata import MetadataQuery, apply_metadata_clause

async def list(self, session, *, metadata: MetadataQuery | None = None, ...):
    statement = repository.get_readable_statement(auth_subject)
    if metadata is not None:
        statement = apply_metadata_clause(Promotion, statement, metadata)
    ...
```
...
**`add_metadata_query_schema`** is called once, at application startup, inside the custom `openapi()` function in `outception/openapi.py`. It injects a reusable `MetadataQuery` component into the global `components/schemas` section of the OpenAPI document:
