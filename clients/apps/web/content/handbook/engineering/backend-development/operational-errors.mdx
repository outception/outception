...
description: "Understanding and handling operational errors in Outception backend development"
---
...
1. **Create a matcher function** in `server/outception/operational_errors.py`:
...
- **API requests**: `OperationalErrorMiddleware` in `server/outception/middlewares.py`
- **Background jobs**: `OperationalErrorMiddleware` in `server/outception/worker/_broker.py`
