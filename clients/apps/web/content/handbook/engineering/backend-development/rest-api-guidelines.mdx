...
description: "Best practices and conventions for designing and implementing RESTful APIs at Outception"
---

This document outlines the guidelines and principles we follow when designing and implementing RESTful APIs at Outception. Adhering to these standards ensures consistency, maintainability, and a better developer experience for both internal and external users of our APIs.
...
- Always use `outception.kit.Schema` (or `TimestampedSchema` when timestamps exist)
...
- Validation errors should return `422 Unprocessable Entity` in FastAPI format via `OutceptionRequestValidationError`
...
**Inherit from `outception.kit.Schema`**

```py
# ✅ DO
from outception.kit import Schema
...
**If the underlying data model includes `created_at` or `updated_at` fields, inherit from `outception.kit.TimestampedSchema`**

```py
# ✅ DO
from outception.kit import TimestampedSchema
...
In most cases, the API supports User and Organization subjects. It's key then when querying resources to filter upon the authentication subject to ensure proper access control.
...
- On validation errors, return `422 Unprocessable Entity` with FastAPI-style details (`OutceptionRequestValidationError`).
