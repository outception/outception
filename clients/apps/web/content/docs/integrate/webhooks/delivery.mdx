...
You now need to setup a route handler for the endpoint registered on Outception to
...
in case you want to roll your own. Validate the signature against the secret you
configured during the endpoint setup before parsing the payload.
...
To re-enable a disabled endpoint, open your organization's [webhook settings](https://outception.com/to/dashboard/settings/webhooks) and manually enable it. Before re-enabling, ensure your endpoint is properly configured and reachable to avoid repeated disabling.

## Troubleshooting

### Not receiving webhooks

Seeing deliveries on Outception, but not receiving them on your end? Below are some
...
- Try adding trailing `/` to the URL on Outception. Often `/foo` is resolved to
  `/foo/` by frameworks.

`HTTP 3xx`

Redirect responses (301, 302, 307, etc.) are treated as failures. Outception does not follow redirects for webhook deliveries. Update your webhook URL to the final destination URL to avoid redirects.
