...
export function GET(request: Request) {
  ...
  return new ImageResponse(
    ...
    {
      ...SIZE,
      // ImageResponse defaults to `max-age=0, must-revalidate`, so every hit
      // re-runs a Satori layout + PNG rasterisation. The output is a pure
      // function of the (validated, length-capped) query params, so it's safe
      // to cache hard — otherwise a trivial request loop is an uncapped CPU
      // sink billed to us.
