...
// Lenis ships its own stylesheet and it is not optional: it releases the
// document's height, keeps a stopped scroller from overflowing, and stops
// iframes swallowing pointer events. Without it a finger's scroll on the
// mosaic was recognised some of the time and not others.
...
import { getLocaleDir, getLocaleMessages } from '@outception-com/i18n'
...
export default async function RootLayout({
  ...
  // Read the locale's strings here and hand them to the client provider.
  // Client components are server-rendered for the first paint, but that render
  // uses a DIFFERENT module instance from this Server Component graph, so
  // loading the chunk here cannot reach it - the first paint of a non-English
  // page was always English, and for ar/he/fa/ur that meant English text under
  // `dir="rtl"`. Passing the payload is what crosses the boundary.
  ...
  return (
    ...
    >
      <body style={{ textRendering: 'optimizeLegibility' }}>
        ...
        {/* Offline fallback: the service worker precaches /offline.html (the
            branded no-connection page) and serves it when a navigation fails
            without network. It caches nothing else. */}
        <script
          // eslint-disable-next-line react/no-danger
          dangerouslySetInnerHTML={{
