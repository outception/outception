...
// Site-wide social-share card. A file-based OpenGraph image auto-emits the
// `og:image` (and, via twitter-image, `twitter:image`) meta tags with the right
// dimensions, so link previews render instead of a blank card. Deliberately
// uses ImageResponse's built-in font - no runtime Google Fonts fetch - so the
// route can never fail to produce a PNG for a crawler.

export const alt =
  'Outception, a live wall of headline cards from 8,000+ sources, across every topic'
export const size = { width: 1200, height: 630 }
export const contentType = 'image/png'

// The brand sparkle mark (same silhouette as LogoIcon) in a fixed red ramp, so
// the share card matches the app icon / social avatar. Inlined as a
// self-contained SVG (no CSS tokens) so the OG renderer can rasterise it.
const STAR_PATH =
  ...
  return new ImageResponse(
    <div
      style={{
        ...
        backgroundImage:
          'radial-gradient(900px 520px at 82% 8%, #e5271f55, transparent 60%), radial-gradient(760px 520px at 0% 108%, #e5271f22, transparent 58%)',
        padding: '72px',
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        ...
        >
          From 8,000+ sources, across every topic. Free to read.
        </div>
      </div>
