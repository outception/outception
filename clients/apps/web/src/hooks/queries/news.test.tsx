...
describe('useNewsSource', () => {
  ...
  it('requests the source with latest=false and the given sort', async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(source).toHaveBeenCalledWith('hackernews', false, 'top')
  })
})

describe('useNewsBatch', () => {
  it('does not fetch for an empty source list', () => {
    const { result } = renderHook(() => useNewsBatch([]), {
      wrapper: makeWrapper(),
    })
    expect(result.current.fetchStatus).toBe('idle')
  ...
  it('fetches the requested sources', async () => {
    batch.mockResolvedValue([{ id: 'hackernews' }])
    const { result } = renderHook(
      () => useNewsBatch(['hackernews', 'reddit']),
      {
        wrapper: makeWrapper(),
      },
    )
