...
describe('useNewsSource', () => {
  ...
  it('requests the source with latest=true and the given sort', async () => {
    source.mockResolvedValue({ id: 'hackernews' })
    const { result } = renderHook(() => useNewsSource('hackernews', 'top'), {
      wrapper: makeWrapper(),
    })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(source).toHaveBeenCalledWith('hackernews', true, 'top', undefined)
  })
  ...
  it('passes the reader locale so headlines come back translated', async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(source).toHaveBeenCalledWith('hackernews', true, 'hot', 'de')
  })
})
