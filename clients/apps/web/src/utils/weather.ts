 ...
 * /news/weather proxy instead of a headline feed. */
...
export const weatherApi = {
  ...
  // default-deck). Precise coordinates win; otherwise the IP country is sent
  // and the backend resolves that country's capital.
  forecast: async (loc: WeatherLocation): Promise<WeatherData> => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/v1/news/weather?${params.toString()}`,
    )
...
// WMO weather-interpretation codes → a short label key and a glyph. Grouped
// per the Open-Meteo docs (drizzle/rain/snow families collapse to one entry).
