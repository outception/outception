...
export const newsApi = {
  ...
  // Headlines for MANY sources in one round trip, read straight from cache.
  // This route never starts a fetch, so it is cheap enough to ask for the
  // whole card set at once - which is what the zoom view needs, since it
  // shows every card on screen together rather than one at a time.
  // Anything not yet translated comes back as-is; the card's own request
  // fills that in.
  batch: (sources: readonly string[], lang?: string) =>
    unwrap(
      api.POST('/v1/news/batch', {
        body: { sources: [...sources], lang },
      }),
    ),
