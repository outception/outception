...
/** Reduce a BCP-47 code to the supported UI locale we translate to. Shared
 * with the app via the i18n package so browser and device detection cannot
 * drift - the two local copies had both been silently dropping every Chinese
 * and Filipino tag, sending those readers to English. */
const toSupportedLocale = (code: string): AcceptedLocale | null =>
  sharedToSupportedLocale(code) as AcceptedLocale | null

/** Request header the proxy sets from a shared card's `?lang=`, so the server
 * render can honour it. A root layout receives no searchParams, and the server
 * and client resolvers must agree or the first paint is in the wrong language.
 */
export const SHARED_LOCALE_HEADER = 'x-outception-lang'
