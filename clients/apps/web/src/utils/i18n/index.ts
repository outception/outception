...
export async function resolveLocale(
  ...
  // A shared card's `?lang=`, forwarded by the proxy because a root layout
  // receives no searchParams. Ranked first, matching the client resolver.
  const shared = (await headers()).get(SHARED_LOCALE_HEADER)
  if (shared && isAcceptedLocale(shared)) {
    return shared
  }
...
export async function metaTitle(
  ...
  return getTranslations(locale).meta[key]
}
