...
vi.mock('@outception-com/i18n', () => {
  ...
  return {
    ...
    // Mirrors the real resolver for this cut-down locale list: exact match,
    // then the bare primary language. The script/region aliases (zh-Hans,
    // fil → tl) are covered by the i18n package's own tests.
    toSupportedLocale: (code: string) => {
      const list = SUPPORTED_LOCALES as readonly string[]
      if (list.includes(code)) return code
      const primary = code.split('-')[0]
      return list.includes(primary) ? primary : null
    },
  }
})
