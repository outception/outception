...
// Catch-all params are URL-decoded after route matching, so an encoded
// `%2e%2e%2f` arrives as a literal `../` inside one segment and path.join
// happily escapes the content root - enough to read the login-gated handbook
// through the public /docs route.
const SAFE_SEGMENT = /^[a-z0-9][a-z0-9._-]*$/i

const isSafeSlug = (slug: string[]): boolean =>
  slug.every((segment) => segment !== '..' && SAFE_SEGMENT.test(segment))
...
export async function readDoc(
  ...
  if (!isSafeSlug(resolved)) return null
