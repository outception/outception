...
import { isAcceptedLocale } from '@outception-com/i18n'
...
import { GEO_COUNTRY_COOKIE, SHARED_LOCALE_HEADER } from './utils/i18n/shared'
...
// The persistent distinct id is an analytics identifier, so it only exists
// once the reader has accepted cookies (the banner mirrors its localStorage
// choice into CONSENT_COOKIE). Everyone else gets a per-request id: server
// experiments still resolve, but nothing identifies the visitor across
// requests.
const getOrCreateDistinctId = (
  request: NextRequest,
): { id: string; isNew: boolean; consented: boolean } => {
  const consented = request.cookies.get(CONSENT_COOKIE)?.value === 'yes'
  const existing = request.cookies.get(DISTINCT_ID_COOKIE)?.value
  if (existing && consented) {
    return { id: existing, isNew: false, consented }
  }
  return { id: `anon_${nanoid()}`, isNew: true, consented }
}
...
export async function proxy(request: NextRequest) {
  ...
  // Forward a shared card's `?lang=` to the server render. A root layout gets
  // no searchParams, so without this the server resolved the reader's OWN
  // language for a link that names another one: it inlined the wrong locale's
  // strings into the HTML (~30 KB never used) and the right one was only
  // fetched after hydration - both costs, on exactly the shared-link path the
  // parameter exists to serve. The client resolver already ranks ?lang first.
  const sharedLang = request.nextUrl.searchParams.get('lang')
  if (sharedLang && isAcceptedLocale(sharedLang)) {
    requestHeaders.set(SHARED_LOCALE_HEADER, sharedLang)
  }

  const response = NextResponse.next({ request: { headers: requestHeaders } })

  if (consented && isNewDistinctId) {
  ...
  } else if (!consented && request.cookies.has(DISTINCT_ID_COOKIE)) {
    // Consent withdrawn (or never given, from before the gate): drop the id.
    response.cookies.delete(DISTINCT_ID_COOKIE)
  }
