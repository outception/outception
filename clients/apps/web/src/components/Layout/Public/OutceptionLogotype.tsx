...
export const OutceptionLogotype = ({
   ...
   * pre-rendered 3D spinning-top animation as it flips). Right-click still opens
   * the brand menu.
   */
  togglesTheme?: boolean
}) => {
  const OutceptionLogotypeRef = useRef<HTMLDivElement>(null)
  const { resolvedTheme, setTheme } = useTheme()

  const [OutceptionLogotypeOpen, setOutceptionLogotypeOpen] = useState(false)
  const [spinning, setSpinning] = useState(false)

  useOutsideClick([OutceptionLogotypeRef], () =>
    setOutceptionLogotypeOpen(false),
  )
  ...
  const handleTriggerClick: MouseEventHandler<HTMLElement> = useCallback(
    (e) => {
      e.preventDefault()
      e.stopPropagation()
      setOutceptionLogotypeOpen(true)
    },
    [],
  )
  ...
  const handleCopyLogoToClipboard = useCallback(() => {
    navigator.clipboard.writeText(
      logoVariant === 'icon'
        ? OutceptionIconSVGString
        : OutceptionLogoSVGString,
    )
    setOutceptionLogotypeOpen(false)
  }, [logoVariant])
  ...
  return (
    <div className={twMerge('relative flex flex-row items-center', className)}>
      ...
      <DropdownMenu open={OutceptionLogotypeOpen}>
        <DropdownMenuTrigger
          className={href || togglesTheme ? 'cursor-pointer' : ''}
        ...
        <DropdownMenuContent ref={OutceptionLogotypeRef} align="start">
          <DropdownMenuItem
          ...
          <DropdownMenuItem
            className="flex flex-row gap-x-3"
            onClick={() => setOutceptionLogotypeOpen(false)}
          >
            <ArrowDown className="h-3 w-3" />
            <Link href="/assets/brand/outception_brand.zip">
...
const OutceptionIconSVGString = `<svg width="300" height="300" viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const OutceptionLogoSVGString = `<svg width="1020" height="300" viewBox="0 0 1020 300" fill="none" xmlns="http://www.w3.org/2000/svg">
