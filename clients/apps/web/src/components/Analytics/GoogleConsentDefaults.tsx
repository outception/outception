...
export function GoogleConsentDefaults() {
  const script = `(function(){window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('consent','default',{analytics_storage:'denied',wait_for_update:500,region:${CONSENT_REGIONS}});gtag('consent','default',{analytics_storage:'granted'});try{var c=localStorage.getItem('cookie_consent');if(c==='yes'){gtag('consent','update',{analytics_storage:'granted'});}else if(c==='no'){gtag('consent','update',{analytics_storage:'denied'});}}catch(e){}})();`
  return (
    <script
