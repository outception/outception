 ...
 * height so the swipe card set can size it.
 *
 * Renders bare (no own surface): the draggable SwipeCard wrapper owns the
...
export const NewsSourceCard = ({
  ...
  const { data, isLoading, isError, errorUpdateCount, dataUpdatedAt } =
    useNewsSource(source.id, 'hot', active, active || upcoming || hasBeenActive)
  ...
  const onItemMenu = useCallback(
    (e: React.MouseEvent, item: NewsItem) => openMenu(e, item, source),
    [openMenu, source],
  )
  const listRef = useRef<HTMLDivElement>(null)
  useClipPartialRows(listRef)

  // A source that keeps failing is dropped from the card set (see NewsWall) so dead
