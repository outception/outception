 ...
 * search dialog styled like the "Cards" source palette; picking a flag sets the
 * language and closes. */
export const LanguagePicker = ({
  open: controlledOpen,
  onOpenChange,
}: {
  /** The navbar drives this so its marker can sit on the flag while the
   * picker is up. Uncontrolled anywhere else. */
  open?: boolean
  ...
  const choose = (locale: SupportedLocale, country?: string) => {
    // A shared card link carries `?lang=<sharer's locale>`, which otherwise
    // outranks an explicit pick (see resolveClientLocale) - so a reader who
    // arrived via a shared link couldn't change the language. Drop the param
    // first so the explicit choice below actually takes effect.
  ...
  return (
    <>
      <button
        type="button"
        aria-label={t('news.language.title')}
        aria-expanded={open}
        onClick={() => setOpen(true)}
        className="flex cursor-pointer items-center gap-1 rounded-xl px-3 py-1 [color:color-mix(in_srgb,var(--color-ink)_55%,transparent)] transition-[color,transform] duration-100 hover:[color:var(--color-ink)] active:scale-95 active:[color:var(--color-ink)] dark:[color:color-mix(in_srgb,var(--color-ink-night)_65%,transparent)] dark:hover:[color:var(--color-ink-night)] dark:active:[color:var(--color-ink-night)]"
      >
        {flagCountry ? (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <Command className="rounded-3xl bg-transparent text-black dark:text-white">
