...
// followed). Distinguishes a fresh visitor pruning the seeded card set - who
// should keep their remaining suggestions - from a curator whose emptied
// card set must stay empty rather than re-seeding.
...
// Whether this device has ever written a card set ('[]' counts). Distinguishes a
// fresh visitor - whose empty set falls back to the seeded default card set - from
// a reader who explicitly emptied their card set ("Deselect all"), which must stay
// empty rather than re-seeding.
const hasStoredCards = (): boolean => {
  try {
...
export const getCardsClearedSnapshot = (): boolean =>
  hasStoredCards() && getFocusedSnapshot().length === 0
...
// card set). Registered by the wall while seeding so the first follow can
// materialize the whole seed into the followed set - otherwise starring one
// seeded source would collapse the card set to just that source.
...
export const toggleFocus = (id: string): void => {
  ...
  // First follow while the wall only shows the seeded default card set: promote the
  // whole seed to the followed set so those sources become the user's real card set
  ...
  const base =
    adding && current.length === 0 && !hasStoredCards() && seedCards.length > 0
      ? seedCards.filter((x) => !getHiddenSnapshot().includes(x))
      : current
  // Newly-followed source goes to the FRONT of the card set so it becomes the top
  // card immediately (the card set jumps to the first newly-added id). Prepending -
  // not appending - is what puts "what you just clicked" on top no matter which
  // card was showing.
  const next = adding
    ? [id, ...base.filter((x) => x !== id)].slice(0, MAX_CARDS)
    : current.filter((x) => x !== id)
  // Record the just-followed source so the card set jumps to it (see
  ...
  try {
    ...
    // storage disabled - the card set just won't persist
  }
  emit()
}

// The source the reader most recently chose to follow, with a monotonic seq so
// the card set can jump to it EVERY time - even when it was already on the wall (a
...
const MAX_BULK_FOLLOW = 60
// Hard ceiling on the card set: every followed id mounts a card window entry and
...
export const followAll = (ids: string[]): void => {
  ...
  // all into localStorage. Take the first slice; the card set stays usable.
  ids = ids.slice(0, MAX_BULK_FOLLOW)
  const current = getFocusedSnapshot()
  const base =
    current.length === 0 && !hasStoredCards() && seedCards.length > 0
      ? seedCards.filter((x) => !getHiddenSnapshot().includes(x))
      : current
  ...
  const next = [...ids, ...base.filter((x) => !idSet.has(x))].slice(
    0,
    MAX_CARDS,
  )
 ...
 * followed set. Writing the emptied list marks the card set as explicitly cleared
 * (see getCardsClearedSnapshot) so the wall shows the empty state instead of
 * re-seeding the default card set. */
...
// both tabs and the seeded default card set - until re-followed from the palette.
