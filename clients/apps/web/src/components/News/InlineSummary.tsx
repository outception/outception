...
// A publisher teaser can arrive in its ORIGINAL language while the server is
// still translating it (`pending`). Ask again a few times and swap the
// translated line in, instead of leaving the panel in English - the app does
// the same on its own /summary route. Bounded, and cache-first on the server,
// so a stuck translation costs a handful of cheap requests at most.
const TEASER_RETRY_MS = 1_500
const TEASER_RETRY_LIMIT = 4

type Event =
  | { text: string; kind?: string; pending?: boolean }
  | { delta: string }
...
export const InlineSummary = ({
  ...
  // Bumping this re-runs the fetch effect below; the counter bounds how often
  // an untranslated teaser may do so (see TEASER_RETRY_LIMIT).
  const [teaserRetry, setTeaserRetry] = useState(0)
  const teaserRetries = useRef(0)
  useEffect(() => {
    teaserRetries.current = 0
  }, [url, locale])
      ...
      .then(async (r) => {
        ...
        await readEvents(r.body, (event) => {
          ...
          if ('text' in event) {
            ...
            if (event.pending && teaserRetries.current < TEASER_RETRY_LIMIT) {
              // Still the publisher's original language: ask again shortly so
              // the reader gets the translated line without re-tapping.
              teaserRetries.current += 1
              teaserTimer = setTimeout(() => {
                if (alive) setTeaserRetry((n) => n + 1)
              }, TEASER_RETRY_MS)
            }
            return
          }
  ...
  }, [url, locale, teaserRetry])

  // The card is a fixed-height clip box, so a summary opened under a
