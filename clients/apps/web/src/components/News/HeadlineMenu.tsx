...
/** The source a headline belongs to, as the menu needs it. */
export type MenuSource = { id: string; name: string }
 ...
 *
 * The source travels with the click rather than with the hook, because the
 * mosaic mixes every followed outlet into one grid - one menu there serves
 * tiles from many sources, while a card's rows all share its own. */
export const useHeadlineMenu = () => {
  const [menu, setMenu] = useState<MenuState>(null)
  const t = useT()

  const openMenu = useCallback(
    (e: React.MouseEvent, item: NewsItem, source: MenuSource) => {
      e.preventDefault()
      // Clamp so the menu never renders off the right/bottom edge.
      const x = Math.min(e.clientX, window.innerWidth - 240)
      const y = Math.min(e.clientY, window.innerHeight - 240)
      setMenu({ item, source, x, y })
    },
    [],
  )
  ...
  const element = menu ? (
    <HeadlineMenuCard
      ...
      x={menu.x}
      y={menu.y}
      sourceId={menu.source.id}
      sourceName={menu.source.name}
      onClose={close}
      t={t}
    />
  ) : null
