...
interface NewsColumnState {
  /** Whether the source-search palette ("Cards" / ⌘K) is open. */
  searchOpen: boolean
  setSearchOpen: (open: boolean) => void
  /** One card at a time, or the whole wall as a mosaic. Shared here for the
   * same reason `searchOpen` is: the control lives in the logo's fan and the
   * rendering lives in the wall body. Not persisted - the mosaic is a way to
   * look over everything, not a home to settle into. */
  view: WallView
  ...
  /** Sources whose feed failed to load - dropped from the card set so dead cards
   * never show. */
...
export type WallView = 'cards' | 'zoom'
...
export const NewsColumnProvider = ({ children }: PropsWithChildren) => {
  ...
  const [view, setView] = useState<WallView>('cards')
  ...
  // the card set shifted underneath the reader.
...
/** The same state, or null when there is no wall. The logo renders in the
 * dashboard layouts too, where there is nothing to switch to, so the fan asks
 * with this and simply omits the mosaic control rather than throwing. */
export const useOptionalNewsColumn = (): NewsColumnState | null =>
  useContext(NewsColumnContext)
