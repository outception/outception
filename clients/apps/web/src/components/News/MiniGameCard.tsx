 ...
 * card set. The header strip stays in the newsprint idiom and doubles as the
 * drag handle for swiping the card away - pointer events inside the iframe
 * belong to the game. The card set mounts a card one swipe before it arrives,
 * so the game is already booted by the time it lands on top (and stays
 * mounted so a swipe away doesn't reset a solve). Staying mounted would
 * otherwise leave the solve clock running while the reader browses elsewhere -
 * the app pauses by unmounting its WebView, so here the host posts `blur` /
 * `focus` as the card leaves and returns to the top and the game holds its
 * ticker to match. Only away-time is affected; nothing else about scoring is.
 *
...
type MiniGameId = keyof typeof MINI_GAMES
...
export const MiniGameCard = ({
  ...
  const send = useCallback((cmd: string) => {
    frameRef.current?.contentWindow?.postMessage(
      ...
      // Same-origin frame: never broadcast commands to a swapped document.
  ...
  useEffect(() => {
    if (!active) return
    setGameStrip({ note, actions, playing, send })
    return () => setGameStrip(null)
  }, [active, note, actions, playing, send])

  // Hold the game's clock whenever this card isn't the one in front, so a
  // detour through the card set can't cost the reader a best time.
  useEffect(() => {
    send(active ? 'focus' : 'blur')
  }, [active, send])
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* The game's clock. A flex sibling, NOT absolutely centered: the
            centered version sat on top of the title whenever a localized name
            ran long ("Crucigrama del NYT" printed straight through "0:02").
            As a shrink-proof sibling the title ellipsizes BEFORE reaching it,
            at the cost of the clock sitting right-of-center. */}
      ...
      >
        <iframe
          ...
          // The effect above fires before the document exists, so the first
          // blur/focus has to wait for the frame to be listening.
          onLoad={() => send(active ? 'focus' : 'blur')}
          style={{
            border: 0,
            width: '100%',
            height: '100%',
