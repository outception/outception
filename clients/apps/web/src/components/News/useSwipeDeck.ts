...
export const useSwipeDeck = (
  ...
  const lastFocusSeq = useRef(focusRequest?.seq ?? 0)
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-shot jump keyed on focusRequest.seq; the guard above prevents any re-run cascade
    if (i >= 0) move(i)
