...
import { isMiniGameId, MiniGameCard } from './MiniGameCard'
...
// unclipped; see NewsCards). Mobile uses a much smaller peek so the
// neighbours barely poke out behind the front card. The card set resolves which
// applies once (see NewsCards) and passes it down, so every card agrees on
// first paint.
export const PEEK_X = 56
export const PEEK_X_MOBILE = 24
 ...
 * the side the card set advances toward - right in LTR, mirrored to the left in RTL
...
export const SwipeCard = ({
  ...
  canNext: boolean
  canPrev: boolean
  /** How far each neighbour peeks out (PEEK_X / PEEK_X_MOBILE). */
  peekX: number
  ...
  // Measured on grab, not read per frame. `useTransform` re-runs on every
  // pointermove and every spring frame, and `offsetWidth` forces a layout
  // flush - while motion is writing x/y/rotate/scale to the same element in
  // that frame. That read-after-write thrashed layout on the wall's primary
  // gesture, with the invalidation rooted at a card holding ~30 rows. The
  // native twin already measures once into a shared value.
  const grabWidth = useRef(0)
  ...
  const dragRotate = useTransform(
    ...
    (v) =>
      ...
      Math.max(
        -1,
        Math.min(v / (grabWidth.current || cardWidth(el.current)), 1),
      ) *
      MAX_ROTATE,
  )
  ...
  const onPointerDown = (e: PointerEvent) => {
    ...
    if (rect) {
      grabDirY.current = e.clientY < rect.top + rect.height / 2 ? 1 : -1
      // One read, for the whole gesture - this rect is already being measured.
      grabWidth.current = rect.width
    }
  }
  ...
  // is never ambiguous with an inner scroll. On touch, `touchAction: pan-y`
  // hands vertical gestures to the page instead (see below), so there the
  // horizontal axis is the one that commits.
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        scale,
        zIndex: slot.z,
        // pan-y, not none: the card covers most of a phone viewport, so
        // claiming every gesture makes the ad unit and nav strip below it
        // unreachable - a vertical drag has to fall through to the page.
    ...
    >
      ...
      />
      {/* A playable card needs both halves to agree: an id this build ships a
          game page for, AND the server still calling that source a game. Either
          one alone (a retired game, or a `game` source we have no page for)
          falls back to the ordinary news card, which is what the app does. */}
      {source.id === PROMOTED_CARD_ID ? (
        <PromotedCard active={isTop} />
      ) : isMiniGameId(source.id) && source.type === 'game' ? (
        <MiniGameCard game={source.id} source={source} active={isTop} />
      ) : source.type === 'heatmap' ? (
