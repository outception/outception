...
// The STORED value keeps its original spelling on purpose. It is written on
// readers' devices, and renaming the string would read as "never opened" and
// silently reset which card they were on.
const NEWS_CARDS_ACTIVE_KEY = 'news-deck-active'

// The last follow request the card set has acted on, kept at module scope: a
// card set that remounts (the wall re-fetches card metadata after a follow) would
...
const readActiveMap = (): Record<string, string> => {
  ...
  try {
    const parsed: unknown = JSON.parse(
      localStorage.getItem(NEWS_CARDS_ACTIVE_KEY) || '{}',
    )
 ...
 * card set resumes on its first paint. */
 ...
 * you left off and adding/reordering sources doesn't yank the card set around.
 */
export const useSwipeCards = (
  items: string[],
  column: string,
  // When set (e.g. arriving from a live promotion), the card set opens on this
  // source id, overriding the persisted position.
  initialActiveId?: string,
  // Bumped whenever the reader follows a source: the card set jumps to that source
  ...
  return {
    ...
    position: index + 1,
    total: items.length,
    // The card set loops, so both directions stay available whenever there is more
