...
// Bumped whenever a locale chunk lands. `useTranslations` reads it through
// useSyncExternalStore, so EVERY component holding a `t` re-renders when the
// real strings arrive - not only the provider. The provider re-rendering alone
// changed nothing for its subtree (same `children` element, same context
// value), so screens that had already painted in English and had no state
// change of their own (the card set tabs, settings rows) stayed English while the
// cards, which re-render when their data lands, switched language: a
// half-translated app.
let translationsVersion = 0

/** How many locale chunks have landed. Pair it with `subscribeTranslations`
 * in a `useSyncExternalStore` INSIDE a client component and hand the result
 * to `useTranslations` - this module is imported by React Server Components
 * too (for `getTranslations`), so it must not depend on a client-only hook
 * itself. */
export const getTranslationsVersion = (): number => translationsVersion
...
/** Register a locale's strings synchronously, from a payload the server
 * already has.
 *
 * Client components are server-rendered for the first paint, and that render
 * reads THIS module instance - a different one from the Server Component
 * graph, so awaiting `loadTranslations` in a server layout does not reach it.
 * Without seeding, the first paint of a non-English page was always English:
 * `getTranslations` found no chunk, returned the English fallback, and the
 * real strings only arrived after hydration downloaded the locale chunk. For
 * ar/he/fa/ur that meant English text under `dir="rtl"`.
 *
 * Idempotent, and a no-op for English or an already-loaded locale, so the
 * client never re-downloads what the server inlined.
 */
export type LocaleMessages = LocaleShape<Translations>

/** A locale's raw strings, for handing across the Server/Client boundary.
 * Returns null for English (the client already has it bundled) and for a
 * chunk that fails to load, so the caller simply passes nothing and the
 * client falls back to its own lazy load. */
export async function getLocaleMessages(
  locale: AcceptedLocale = DEFAULT_LOCALE,
): Promise<LocaleShape<Translations> | null> {
  const translationLocale = getTranslationLocale(locale)
  if (translationLocale === DEFAULT_LOCALE) return null
  try {
    const mod =
      await localeLoaders[translationLocale as Exclude<SupportedLocale, 'en'>]()
    return mod.default as LocaleShape<Translations>
  } catch {
    return null
  }
}
...
export function seedTranslations(
  ...
  payload: LocaleShape<Translations>,
): void {
  const translationLocale = getTranslationLocale(locale)
  if (translationLocale === DEFAULT_LOCALE) return
  if (loadedLocales.has(translationLocale)) return
  translations[translationLocale] = payload as never
...
const isAtomicLeaf = (v: unknown): boolean =>
  v === null || typeof v !== 'object' || '_mode' in v
...
export const useTranslations = (
  locale: AcceptedLocale,
  /** `getTranslationsVersion()`, subscribed per consumer by the caller (see
   * each app's `useT`): a new `t` identity when a chunk lands is what makes
   * memoised callers re-read their strings. Defaults to 0 for callers that
   * do not care about the lazy-load swap. */
  version = 0,
): TranslateFn => {
  return useCallback(
    ...
    // `version` is not read inside: it is here so the callback identity
    // changes when a chunk lands and memoised consumers re-run.
    [locale, version],
  )
}
