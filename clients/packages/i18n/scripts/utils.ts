...
/**
 * Attach translator notes (see src/locales/en.context.ts) to the matching
 * source entries so prepareForLLM can pass them along. Unknown keys are ignored.
 */
export function applyLlmContext(
  sourceKeys: Map<string, EntryValue>,
  context: Record<string, string | undefined>,
): void {
  for (const [key, note] of Object.entries(context)) {
    const entry = sourceKeys.get(key)
    if (entry === undefined || note === undefined) continue
    sourceKeys.set(key, { value: getStringValue(entry), _llmContext: note })
  }
}
...
// Placeholders AND quote characters are masked before the text is sent.
//
// The quotes matter because the reply comes back as a JSON array. A source
// string that marks a label with quotes ("Open “Cards” to follow publications")
// gets rendered by the model with plain double quotes around the translated
// label, unescaped, which ends the JSON string early and makes the whole reply
// unparseable. It did that every time, so no retry could help: one locale then
// fell through to the keyless Google endpoint, which rate limits CI runners,
// and three consecutive runs died there after ~30 languages of real work.
//
// Masking them means the model never sees a quote and never writes one; the
// original characters are restored afterwards.
export const maskPlaceholders = (
  s: string,
...
export const restorePlaceholders = (s: string, tokens: string[]): string =>
