...
async function validate() {
  ...
  for (const locale of targetLocales) {
    ...
    // A locale only ever needs the ordinal categories its own CLDR rules can
    // select: French ordinals are one|other, German/Japanese/Korean are other
    // only. Demanding `ordinal.zero`/`ordinal.many` from them asks for strings
    // that `formatOrdinal` can never reach — it resolves the category and falls
    // back to `other`.
