...
describe('prepareForLLM', () => {
  ...
  it('applyLlmContext annotates matching source keys only', () => {
    const sourceKeys = new Map<string, EntryValue>([
      ['ordinal.one', 'st'],
      ['news.tabs.more', 'Sources'],
    ])
    applyLlmContext(sourceKeys, {
      'ordinal.one': 'Ordinal suffix',
      'missing.key': 'ignored',
    })

    expect(sourceKeys.get('ordinal.one')).toEqual({
      value: 'st',
      _llmContext: 'Ordinal suffix',
    })
    expect(sourceKeys.get('news.tabs.more')).toBe('Sources')
    expect(sourceKeys.has('missing.key')).toBe(false)
  })
