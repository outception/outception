...
/** Attempts per request before a language is given up on. */
const GOOGLE_MAX_ATTEMPTS = 5

/** Whole-locale attempts against Claude before falling back. */
const CLAUDE_LOCALE_ATTEMPTS = 3
...
async function googleTranslateOne(
  ...
  // This endpoint is keyless and rate limited, and it is the LAST line: when
  // Claude has already declined a chunk there is nothing behind it. Throwing
  // straight away on a 429 threw away every language translated so far - a
  // full run is ~45 minutes - so wait it out instead. Exponential with
  // jitter, honouring the server's own retry-after when it sends one.
  if (!res.ok) {
    ...
    if (retryable && attempt < GOOGLE_MAX_ATTEMPTS) {
      ...
      await sleep(wait)
      return googleTranslateOne(text, tl, attempt + 1)
    }
    throw new Error(`Google translate ${res.status} (tl=${tl})`)
  }
...
async function googleTranslateBatch(
  ...
  // Sequential, not Promise.all: firing a request per string at once is a
  // burst against a rate-limited endpoint, which is what provokes the 429
  // this path exists to survive.
  const out: string[] = []
  for (const t of texts) {
    out.push(await googleTranslateOne(t, tl))
    await sleep(120)
  }
  return out
}
...
async function callLLM(
  ...
  if (claudeEnabled()) {
    ...
    // Ask again before giving up on the language. A malformed reply is
    // stochastic, so a fresh attempt usually lands - and the fallback below is
    // the keyless Google endpoint, which rate limits CI runners hard enough
    // that it is effectively unavailable there. Two runs died at ~30 languages
    // because one bad reply sent a single locale down a path that could not
    // work, and that threw away every language already translated.
    for (let attempt = 0; attempt < CLAUDE_LOCALE_ATTEMPTS; attempt++) {
      try {
        translated = await claudeTranslateBatch(masked, language)
        break
      } catch (error) {
        const last = attempt === CLAUDE_LOCALE_ATTEMPTS - 1
        log.warning(
          `Claude translation failed for ${language} (${(error as Error).message})` +
            (last ? '; falling back to Google' : '; retrying'),
        )
        if (!last) await sleep(2_000 * (attempt + 1))
      }
    }
  }
  if (translated === null) {
