---
name: render-env
description: Add a new Terraform Cloud variable for the Render-hosted backend. Declares the tfe_variable in terraform/global/production.tf and the matching variable {} block in terraform/production/variables.tf, then reminds the user to wire it into render.tf / the render_service module.
user-invocable: true
...
- The TFC `key` is the bare name: `key = "${name}"` — **no** `_production` suffix on the key. The variable set is per-workspace so the key doesn't need to be globally unique.
- The `tfe_variable` resource label gets the env suffix: `resource "tfe_variable" "${name}_production"`.
- The matching `variable` block in `terraform/production/variables.tf` uses the bare name: `variable "${name}"`. This lets `render.tf` reference it as `var.${name}`.

A handful of older variables (e.g. `google_client_id_production`, `backend_secret_production`) use a `_production`-suffixed key and matching variable. Don't replicate that pattern for new additions — it's legacy.

## Step 1: Add the tfe_variable to global/production.tf

Append a block to `terraform/global/production.tf` (after the existing `tfe_variable` resources, before the file ends):

```hcl
resource "tfe_variable" "${name}_production" {
  key             = "${name}"
  category        = "terraform"
...
## Step 2: Add the variable {} block to production/variables.tf

Append a block to `terraform/production/variables.tf`:
...
- That the variable is now declared but **not yet consumed**. To consume it, they need to:
  ...
  2. **Wire the field into the matching `render_env_group` block in `terraform/modules/render_service/main.tf`** as `OUTCEPTION_${NAME_UPPER} = { value = var.<object>.<field> }`. Two backend groups exist:
     - `render_env_group "backend"` — the base backend group.
     - `render_env_group "backend_production"` — production-specific values (e.g. `OUTCEPTION_PLAIN_TOKEN`).
  3. **Pass the value in from `terraform/production/render.tf`** module call, e.g. `backend_secrets = { ... ${field} = var.${name} ... }`.
  4. **Set the actual value in TFC** under the Production variable set.
