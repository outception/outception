...
description: "Learn how to grant meter credits to customers before they make any purchase using the Outception API."
---
...
1. Create a customer in Outception (if they don't exist)
...
<Steps>
  <Step title="Navigate to Meters">
    Open [**Products → Meters**](https://outception.com/to/dashboard/products/meters) in the dashboard.
  </Step>
...
If your customer doesn't already exist in Outception, you need to create them first.

### Option A: Create via Dashboard

<Steps>
  <Step title="Navigate to Customers">
    Open [**Customers**](https://outception.com/to/dashboard/customers) in the dashboard.
  </Step>
...
Use the Outception SDK or API to create a customer programmatically:
...
<Tip>
  Use the `externalId` field to link Outception customers with your internal user system. This allows you to reference customers without storing Outception's internal ID.
</Tip>

### Using External ID

If you set an `externalId` when creating the customer, you can use it in event ingestion instead of the Outception customer ID:
