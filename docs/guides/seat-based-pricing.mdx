...
With standard Outception products, one person buys and one person uses — they're the same person. With seat-based products, buying and using are separate concerns, modeled through three distinct entities:
...
- **Benefits are not granted at purchase time.** Seats must be assigned and claimed before benefits are granted. (Exception: when using the default Outception confirmation page, the buyer's seat is auto-claimed.)
...
- Outception organization with seat-based pricing enabled
- Outception SDK installed (`npm install @polar-sh/sdk` or `pip install polar-sdk`) — make sure it's the latest version
- Basic understanding of Outception products and subscriptions
...
<Steps>
  <Step title="Navigate to Products">
    Open the dashboard and [create a new product](https://outception.com/to/dashboard/products/new).
  </Step>
...
Outception automatically sends invitation emails to assigned team members with a secure claim link. Once a member claims their seat, benefits are granted immediately.

<Info>
Invitation emails are sent automatically when seats are assigned. The claim flow is fully managed by Outception — members click the link, claim their seat, and get immediate access to benefits.
</Info>
...
- **Communicate clearly** to billing managers about seat assignment: if using the default Outception confirmation page, their seat is auto-claimed; if using a custom success URL, they'll need to assign themselves a seat through the portal
