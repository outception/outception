---
title: "Integrate Outception with Encore"
sidebarTitle: "Encore"
description: "In this guide, we'll show you how to integrate Outception with Encore."
---
...
Because Encore defines infrastructure in code and provides built-in distributed tracing, AI coding assistants can both build and debug your backend end-to-end, including your Outception integration. They can read the code to understand the full system, then inspect traces to diagnose exactly what happened when a checkout completes or a webhook arrives.

Consider following this guide while using the Outception Sandbox Environment. This will allow you to test your integration without affecting your production data.

## Examples

- [With Encore](https://github.com/outception/examples/tree/main/with-encore)

## Install the Outception JavaScript SDK

To get started, install the Outception JavaScript SDK and the webhook verification library:
...
### Outception Access Token and Mode

To authenticate with Outception, you need to create an access token and specify if it's production or sandbox mode. You can create an [organization access token from your organization settings](/integrate/oat).
...
## Configuring a Outception API Client

To interact with the Outception API, create a new instance of the `Polar` class using Encore's secret management.
...
## Generating Outception Checkout Sessions

Create a checkout endpoint that creates a Outception checkout session and redirects the user.
...
## Handling Outception Webhooks

Outception can send you events about various things happening in your organization. This is useful for keeping your database in sync with Outception checkouts, orders, subscriptions, etc.
...
Encore runs your app locally with full infrastructure support. To receive webhooks during local development, you can use the [Outception CLI](https://outception.com/docs/integrate/webhooks/locally) to tunnel webhook events to your local environment.
...
1. Point the Webhook to `your-app.com/webhooks/polar`. This must be an absolute URL which Outception can reach. If you use the Outception CLI tunnel, it will handle this for you.
...
3. Generate a secret key to sign the requests. This will allow you to verify that the requests are truly coming from Outception.
...
<Warning>
  The `standardwebhooks` library expects the secret to be base64-encoded. Outception provides a raw string (starting with `polar_whs_`), so you must base64-encode the entire secret before passing it to `new Webhook()`.
</Warning>
...
2. Update the `server` parameter from `"sandbox"` to `"production"` in your Outception client
3. Update your webhook URL in Outception to point to your production domain
