...
description: Stay up to date with the latest changes and improvements to Outception.
---
...
<img class="border rounded" src="/assets/changelog/2026-04-22/pending-subscription-update.png" />
...
<img class="border rounded" src="/assets/changelog/2026-04-22/license-keys-filter.png" />
...
<img class="border rounded" src="/assets/changelog/2026-04-14/meter-units.png" />
...
<img class="border rounded" src="/assets/changelog/2026-03-31/customizable-dashboard-charts.png" />
...
A new Feature Flag benefit type is available for SaaS merchants. Grant it to customers when they should unlock something in your product, then check entitlements through the Outception API or webhooks. JSON metadata stays invisible to the customer, keeping the experience clean for pure software subscriptions.
...
* Member sessions (`outception_mst_`) preserve identity and permissions
...
* Fixed Outception logo not linking to home
...
* Updated Outception logo in all emails
...
New dedicated downloads page at outception.com/downloads for easy access to desktop and mobile apps.
...
Outception mobile app added to the monorepo:
...
<Update label="2025-10-17">
  ...
  Merchants can now [update existing subscriptions](https://outception.com/docs/api-reference/subscriptions/update#subscriptionupdateproduct) from archived pricing schemes to current ones within the same product.
  ...
  From **October 27th, 2025**, [new IP ranges](https://outception.com/docs/integrate/webhooks/delivery#ip-allowlist) will be added.

  ## Improved Subscription Cancellation Flow

  [Benefits](https://outception.com/docs/features/benefits/introduction) attached to the subscription [are now automatically revoked](https://github.com/outception/outception/pull/7271) when the subscription is canceled.

  ## Ability to specify External ID

  [External ID can now be specified](https://github.com/outception/outception/pull/7275) during creation of a customer via the dashboard.
  ...
  A Return URL can now be set while generating a [checkout session](https://outception.com/docs/api-reference/checkouts/create-session#body-return-url-one-of-0) or a [customer portal session](https://outception.com/docs/api-reference/customer-portal/sessions/create#body-one-of-0-return-url-one-of-0). This allows you to preserve the context for the end users who visit either if they wish to go back to the application.

  ## Ability to search Customers via their External ID

  The [List Customers API](https://outception.com/docs/api-reference/customers/list) now accepts Customer's External ID [in the **query** parameter](https://outception.com/docs/api-reference/customers/list#parameter-one-of-0).
...
We're excited to introduce a chat widget to the Outception dashboard, making it easier than ever for merchants to get help directly within the product.
...
We've updated our webhooks server implementation to [include a timestamp in each payload](https://github.com/outception/outception/pull/6770), in line with the Standard Webhooks specification.
...
Customer emails now appear to come from your organization (e.g., "YourOrg (via Outception)") with replies directed to your organization's email address, providing a more professional and branded experience for your customers.
...
This feature is both available through the API and the dashboard.
...
We're excited to announce the official launch of our [Zapier integration](https://zapier.com/apps/outception/integrations)! Get started now and connect Outception to 2,000+ other web services.

<Note>
  We've focused on **triggers** (webhooks) for now, so you can react to events in Outception and trigger actions in other apps.

Need to perform actions in Outception? Tell us about your use case [here](https://github.com/orgs/outception/discussions/new?category=integrations&labels=integrations%2Fzapier) and we'll consider adding more actions in the future.
...
thrilled to have shipped a Outception plugin for Better Auth - in collaboration with them.
...
We've added support for an `external_id` field on Customers. We believe this will greatly simplify the reconciliation process between your system and Outception.
...
With `external_id`, you can now fetch a Customer directly by their external ID through dedicated `GET`, `PATCH`, and `DELETE` endpoints. You don't even need to store Outception's internal ID in your system anymore! Read more

Of course, you can also directly preset `external_customer_id` when creating a Checkout Session, and it will automatically be set on the newly created Customer after a successful checkout. Read more

</Update>

<Update label="2025-02-19">
  ## Outception's take on Product variants
