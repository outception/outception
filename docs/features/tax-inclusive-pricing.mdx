...
When displaying a price to a customer, there are two common conventions: either the displayed price **already includes tax** (inclusive), or tax is **calculated and added on top** at checkout (exclusive). Outception gives you fine-grained control over this behavior at both the organization level and on individual product prices.
...
| Option             | Description                                                                                        |
| ------------------ | -------------------------------------------------------------------------------------------------- |
| **Location-based** | Outception automatically picks the right behavior based on the customer's country. This is the default. |
...
When the tax behavior is set to **Location-based**, Outception resolves the actual inclusive/exclusive behavior at checkout time based on the customer's billing address:
...
Regardless of the tax behavior chosen, Outception always:
