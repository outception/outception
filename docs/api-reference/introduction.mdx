...
Outception API has rate limits to ensure fair usage and maintain performance. Limits differ between the **Sandbox** and **Production** environments.
...
Unauthenticated validation, activation, and deactivation endpoints are limited to **3 requests per second** in both environments.
