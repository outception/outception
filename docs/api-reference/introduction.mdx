...
<CardGroup cols={2}>
  ...
  <Card title="Sandbox Base URL" icon="flask">
    `https://sandbox-api.outception.com/v1`
  </Card>
...
| ----------- | -------------------------------------- | ------------------------------- |
| Production  | `https://api.outception.com/v1`         | Live data & promotions          |
| Sandbox     | `https://sandbox-api.outception.com/v1` | Safe testing & integration work |

<Info>
  The sandbox environment is fully isolated—data, users, and tokens created
  there do not affect production. Create separate tokens in each environment.
</Info>
...
Most read endpoints (news, public promotions) are open. Organization-scoped
...
```bash curl (Sandbox)
curl https://sandbox-api.outception.com/v1/promotions/featured \
  -H "Accept: application/json"
```
...
The Outception API has rate limits to ensure fair usage and maintain performance. Limits differ between the **Sandbox** and **Production** environments.
...
- **100 requests per minute** per organization or OAuth2 client.
