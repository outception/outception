...
```
https://outception.com/oauth2/authorize?
  response_type=code
  &client_id=CLIENT_ID
...
<ParamField path="response_type=code" type="string" required>
  ...
  the only one supported by Outception.
</ParamField>
...
If you redirect the user to this URL, they'll see a page asking them to grant access to their data, corresponding to the scopes you asked for. By default, Outception generates user-scoped access tokens, so the token will be tied to the user granting access.
...
{
  "token_type": "Bearer",
  "access_token": "outception_at_XXX",
  "expires_in": 864000,
  "refresh_token": "outception_rt_XXX",
  "scope": "openid email",
  "id_token": "ID_TOKEN"
}
```
...
By default, Outception OAuth2 flow generates **user-scoped** access tokens. These tokens are tied to the user granting access, allowing requests to operate on the data of the organizations they are a member of.

```
https://outception.com/oauth2/authorize?response_type=code&client_id=outception_ci_j3X95_MgfdSCeCd2qkFnUw&redirect_uri=https%3A%2F%2Fexample.com%2Fcallback&scope=openid%20email
```
...
```
https://outception.com/oauth2/authorize?response_type=code&client_id=outception_ci_j3X95_MgfdSCeCd2qkFnUw&redirect_uri=https%3A%2F%2Fexample.com%2Fcallback&scope=openid%20email&sub_type=organization
```
...
curl -X GET https://api.outception.com/v1/oauth2/userinfo \
  -H 'Authorization: Bearer outception_at_XXX'
```
