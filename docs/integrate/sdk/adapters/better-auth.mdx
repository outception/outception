...
A [Better Auth](https://github.com/better-auth/better-auth) plugin for integrating [Outception](https://outception.com) payments and subscriptions into your authentication flow.
...
- [Handle Outception Webhooks securely with signature verification](#webhooks-plugin)
- [Customer Portal](#portal-plugin)

## Examples

- [With Next.js, Better Auth and Cloudflare Workers](https://github.com/outception/examples/tree/main/with-nextjs-better-auth-cloudflare-workers)

## Installation

Install the required Better Auth and Outception packages using the following command:
...
## Integrate Outception with BetterAuth

<Steps>
  <Step title="Configure Outception Access Token">
    Go to your Outception Organization Settings, create an Organization Access Token, and add it to the environment variables of your application.
  ...
  <Step title="Configure BetterAuth Server">
    The Outception plugin comes with it's own set of plugins to add functionality to your stack:
    ...
    - **webhooks** - Listen for relevant Outception webhooks

    ```typescript icon="square-js" auth.ts
    ...
    #### Outception Plugin Configuration Options
    ...
    - `client` (required): Outception SDK client instance
    - `createCustomerOnSignUp` (optional): Automatically create a Outception customer when a user signs up
    - `getCustomerCreateParams` (optional): Custom function to provide additional customer creation metadata
    - `use` (optional): Array of Outception plugins to enable specific functionality (checkout, portal, usage, and webhooks)

  </Step>
  <Step title="Configure BetterAuth Client">
    You will be using the BetterAuth Client to interact with the Outception functionalities.

    ```typescript icon="square-js" auth-client.ts
...
Enable the `createCustomerOnSignUp` [Outception plugin configuration option](#outception-plugin-configuration-options) to automatically create a new Outception Customer when a new User is added in the BetterAuth database.

All new customers are created with an associated `externalId`, i.e. the ID of your User in the Database. This skips any Outception to User mapping in your database.

## Sync Customer deletion

To add user deletion logic with external Outception customer deletion in BetterAuth, extend the user config of your betterAuth setup with the deleteUser option and an afterDelete hook. Here’s how to integrate this alongside your Outception plugin and automatic customer creation:
...
[Source code](https://github.com/outception/outception-adapters/blob/main/packages/outception-betterauth/src/plugins/checkout.ts)
...
<Steps>
  <Step title="Use Checkout Plugin">
    Update the `use` property of the Outception plugin for BetterAuth client to have the `checkout` plugin.
  ...
  <Step title="Create checkouts using BetterAuth client">
    ...
    - **`products`** (optional): An array of Outception Product IDs.
...
[Source code](https://github.com/outception/outception-adapters/blob/main/packages/outception-betterauth/src/plugins/usage.ts)
...
Outception's Usage Based Billing builds entirely on event ingestion. Ingest events from your application, create Meters to represent that usage, and add metered prices to Products to charge for it.
...
Because `createCustomerOnSignUp: true` sets the Outception customer's `externalId` to the BetterAuth user ID, you can pass `externalCustomerId: session.user.id` when ingesting — no extra lookup is needed.

The example below uses the Outception SDK directly from a server route handler. The same `polarClient` instance you configured in `auth.ts` is reused:

```typescript icon="square-js" app/api/ai/video/route.ts (Next.js App Router)
...
The `usage` plugin also registers a BetterAuth endpoint that is callable from the client as `authClient.usage.ingestion(...)`. It forwards to Outception from the BetterAuth server and attaches the authenticated user automatically:
...
[Source code](https://github.com/outception/outception-adapters/blob/main/packages/outception-betterauth/src/plugins/webhooks.ts)

The `webhooks` plugin can be used to capture incoming events from your Outception organization.

To set up the Outception `webhooks` plugin with the BetterAuth client, follow the steps below:

<Steps>
  <Step title="Configure Webhook Endpoints in Outception">
    Configure a Webhook endpoint in your Outception Organization Settings page by following [this guide](/integrate/webhooks/endpoints). Webhook endpoint is configured at `/api/auth/polar/webhooks`.
  </Step>
...
The `webhooks` plugin allows you to invoke handlers for all Outception webhook events:
...
[Source code](https://github.com/outception/outception-adapters/blob/main/packages/outception-betterauth/src/plugins/portal.ts)
...
The following method will redirect the user to the Outception Customer Portal, where they can see their orders, purchases, subscriptions, benefits, etc.
...
Learn more about the Outception Customer State [in the Outception Docs](https://outception.com/docs/integrate/customer-state).
