...
Seamlessly integrate Outception.sh subscriptions and payments into your Laravel application. This package provides an elegant way to handle subscriptions, manage recurring payments, and interact with Outception's API. With built-in support for webhooks, subscription management, and a fluent API, you can focus on building your application while we handle the complexities of subscription billing.

<Note>
This provider is not maintained or officially supported by Outception. Use at your own discretion. If you have questions about the provider, please contact the project maintainer.
</Note>
...
Configure your access token. Create a new token in the Outception Dashboard > Settings > Developers and paste it in the `.env` file.

- https://sandbox.outception.com/dashboard/ORG_SLUG/settings (Sandbox)
- https://outception.com/dashboard/ORG_SLUG/settings (Production)
...
Configure your webhook secret. Create a new webhook in the Outception Dashboard > Settings > Webhooks.

- https://sandbox.outception.com/dashboard/ORG_SLUG/settings/webhooks (Sandbox)
- https://outception.com/dashboard/ORG_SLUG/settings/webhooks (Production)
...
Outception includes a JavaScript script that you can use to initialize the [Outception Embedded Checkout](https://outception.com/docs/features/checkout/embed). If you going to use this functionality, you can use the `@polarEmbedScript` directive to include the script in your views inside the `<head>` tag.
...
This package includes a webhook handler that will handle the webhooks from Outception.
...
This package includes a list of commands that you can use to retrieve information about your Outception account.
...
Customers can update their personal information (e.g., name, email address) by accessing their [self-service customer portal](https://outception.com/docs/features/customer-portal). To redirect customers to this portal, call the `redirectToCustomerPortal()` method on your billable model (e.g., the User model).
...
Outception does not offer immediate cancellation. To resume a subscription while it is still in its grace period, use the resume method.
...
The [Outception documentation](https://outception.com/docs/integrate/webhooks/events) includes an example payload.
...
Please see [CHANGELOG](https://github.com/danestves/laravel-outception/blob/main/CHANGELOG.md) for more information on what has changed recently.

## License

The MIT License (MIT). Please see [License File](https://github.com/danestves/laravel-outception/blob/main/LICENSE.md) for more information.
