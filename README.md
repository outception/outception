...
topic — promotion payments are handled by an external payment provider.
...
- **Paid promotions** — signed-in users buy a category's featured slot for a
  ...
  surface as "Promoted" cards on the wall. Payment runs through an **external
  payment provider**, confirmed via a signed webhook.

## Features

- Public news wall (web + mobile) with topic columns and per-topic promotions
- Login-gated promotion compose → hosted checkout → signed webhook
...
```bash
# Promotions via the external payment gateway
OUTCEPTION_PROMOTION_PRODUCT_ID=""          # pay-what-you-want product on your payment-gateway org
