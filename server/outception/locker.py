...
class LockerError(OutceptionError):
    def __init__(
        self,
...
class Locker:
    ...
    async def lock(
        ...
        *,
        ...
        Args:
            name: Name of the lock. Automatically prefixed by `outceptionlock:`.
        ...
        with logfire.span(
            ...
            if not acquired:
                # A zero wait is an overlap probe (cron ticks): losing it is
                # the expected outcome, not an incident.
                report = log.info if blocking_timeout == 0 else log.error
                report(
