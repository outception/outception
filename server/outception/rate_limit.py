...
_BASE_RULES: dict[str, Sequence[Rule]] = {
    ...
    # Public, unauthenticated upstream proxies (Open-Meteo / translation). Each
    # distinct input is a fresh upstream call + Redis key, so bound them tightly
    # to stop cache-cardinality abuse and getting our egress IP rate-limited.
    "^/v1/news/(weather|translate)": [
        Rule(minute=60, block_time=300, zone="news-proxy"),
        Rule(
            group=RateLimitGroup.pending_auth,
            minute=60,
            block_time=300,
        ...
        Rule(group=RateLimitGroup.web, minute=120, block_time=300, zone="news-proxy"),
        Rule(
