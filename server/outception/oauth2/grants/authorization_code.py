...
from outception.authz.repository import select_user_org_ids
...
class CodeChallenge(_CodeChallenge):
    ...
    # Pin the default too: authlib inherits DEFAULT_CODE_CHALLENGE_METHOD =
    # "plain", so a request that sends code_challenge WITHOUT
    # code_challenge_method is stored with a null method and verified as "plain"
    # — silently bypassing the S256-only restriction above. Defaulting an
    # omitted method to S256 keeps verification fail-closed (a plain challenge
    # simply won't match), so the downgrade path is closed.
