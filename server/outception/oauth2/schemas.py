...
from outception.auth.scope import (
    SCOPES_SUPPORTED,
...
# Anchored: matches exactly "localhost" or "*.localhost", NOT
# "localhost.attacker.com" (which an unanchored match would accept, letting a
# public host masquerade as loopback and register a cleartext-http redirect).
_LOCALHOST_HOST_PATTERN = re.compile(r"^(?:[^.]+\.)?localhost$", flags=re.IGNORECASE)


def _is_localhost(host: str) -> bool:
    try:
        # True loopback only (127.0.0.0/8, ::1) — RFC1918 LAN addresses like
        # 10.x / 192.168.x are NOT loopback and must still require HTTPS.
