...
class ClientRegistrationEndpoint(_ClientRegistrationEndpoint):
    ...
    def save_client(
        ...
        # Store the secret hashed. authlib already copied the plaintext into the
        # registration response body before calling save_client, so the client
        # still receives the usable secret exactly once; only the hash persists.
        client_info = dict(client_info)
        raw_secret = client_info.get("client_secret")
        if raw_secret:
            client_info["client_secret"] = OAuth2Client.hash_client_secret(raw_secret)
...
class ClientConfigurationEndpoint(_ClientConfigurationEndpoint):
    ...
    def create_read_client_response(
        ...
        if isinstance(body, dict):
            # The secret is stored hashed, so the plaintext can't be re-served on
            # read. RFC 7592 permits omitting it — the client keeps the value it
            # received once at registration.
            body.pop("client_secret", None)
