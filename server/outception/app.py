...
def configure_cors(app: FastAPI) -> None:
    ...
    # External API calls CORS configuration. Only the public news-wall surface
    # is meant for foreign origins, and it is read-only (POST covers the
    # translate/summary bodies) - the follows PUT/DELETE ride the web app's
    # own origin, which the credentialed config above matches first.
...
def create_app() -> FastAPI:
    ...
    # add_middleware PREPENDS: each registration wraps everything registered
    # before it, so earlier registrations end up INNER. CORS is registered
    # LAST (outermost) so that EVERY response carries its headers - including
    # the limiter's 429, MaxBodySize's 413 and OperationalError's 503, which a
    # browser otherwise reports as an opaque CORS failure with no way to read
    # Retry-After. SecurityHeadersMiddleware sits between the limiter and CORS
    # (registered after the limiter, before CORS), so those same error
    # responses still carry the security headers.
    ...
    # /.well-known - OAuth2/OIDC discovery, JWKS and the Apple domain
    # association. All of it is account machinery, so it follows the same gate
    # as the auth routers: with accounts off it must not exist. It also can't
    # stay ungated because /openid-configuration takes a session from the
    # size-1 sync pool and matches no rate-limit rule, so a few dozen concurrent
    # requests would exhaust the pool and stall every sync handler in the
    # process - taking the public news wall down with it.
