    ...
    try:
        for record in event.get("Records", []):
            ...
            try:
                ...
                log.error(
                    "outception.worker.sqs_task_failed",
...
def _apply_retry_backoff(record: dict[str, Any], exception: BaseException) -> bool:
    ...
    try:
        ...
        if action is RetryAction.DEAD_LETTER:
            ...
            log.info(
                "outception.worker.sqs_retry_exhausted",
                actor=actor,
        ...
        if action is RetryAction.SCHEDULE:
            ...
            log.info(
                "outception.worker.sqs_retry_scheduled_eventbridge",
                actor=actor,
                receive_count=receive_count,
        ...
        log.info(
            "outception.worker.sqs_retry_scheduled",
            actor=actor,
            receive_count=receive_count,
            backoff_seconds=delay_seconds,
        )
        ...
        log.error("outception.worker.sqs_backoff_failed", exc_info=True)
        return True
