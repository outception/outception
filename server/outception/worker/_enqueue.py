...
_job_queue_manager: contextvars.ContextVar["JobQueueManager | None"] = (
    contextvars.ContextVar("outception.job_queue_manager")
)
...
class JobQueueManager:
    ...
    def enqueue_job(
        ...
        **kwargs: JSONSerializable,
        ...
        log.debug("outception.worker.job_enqueued", actor=actor, delay=delay)
...
def enqueue_job(
    ...
    **kwargs: JSONSerializable,
    ...
    """Enqueue a job by actor name; `delay` is in milliseconds."""
...
def make_bulk_job_delay_calculator(
    ...
    *,
    ...
    """Return index -> delay_ms for spreading a bulk enqueue over time.

    Jobs are spaced `target_delay_ms` apart, compressed (down to `min_delay_ms`)
    to fit `max_spread_ms`; past that floor, `allow_spill` either lets the total
    exceed `max_spread_ms` or batches several items onto the same delay.
    """
