...
import dramatiq
import structlog
from dramatiq.asyncio import EventLoopThread, get_event_loop_thread
...
class _EventLoopWatchdog(threading.Thread):
    ...
    def _get_event_loop_stack(self) -> str:
        ...
        # Use the loop thread's own ident rather than matching by name —
        # dramatiq's EventLoopThread doesn't set a stable thread name.
        loop_thread_id = self.loop_thread.ident
...
class MonitoredAsyncIO(AsyncIO):
    ...
    def before_worker_boot(
        ...
        self._watchdog = _EventLoopWatchdog(event_loop_thread)
