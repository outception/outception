    ...
    def _main_loop(self) -> None:
        ...
        while self.state != STATE_STOPPED:
            with logfire.span("Scheduler wakeup"):
                # Cap the sleep well under the health check's 60s staleness
                # threshold: sleeping until the next cron (up to hours for the
                # midnight jobs) let the heartbeat go stale and /healthz
                # flapped 503 on a perfectly healthy worker.
                # _process_jobs returns None when nothing is scheduled or
                # the scheduler is paused (stock code hands that to wait() as
                # "block forever") - min(None, 25) would TypeError and kill
                # every cron plus the very heartbeat this cap protects.
                self._event.wait(
                    min(wait_seconds, 25) if wait_seconds is not None else 25
                )
                self._event.clear()
