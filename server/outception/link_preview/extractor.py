...
def is_fetchable(url: str) -> bool:
    ...
    if host == "169.254.169.254" or host == "metadata.google.internal":
        return False
    try:
        # Resolve BOTH families (A + AAAA) and reject if ANY resolved address
        # is non-global — an IPv6-only internal host would slip past an
        # IPv4-only gethostbyname. (A DNS-rebinding attacker can still return
        # a different IP at connect time; fully closing that needs IP pinning
        # or egress restrictions — deferred.)
    ...
    for info in infos:
        try:
            ...
            return False
        # Reject private / loopback / link-local / multicast / reserved ranges
        # (and IPv6 equivalents). `is_global` covers them in one check.
        if not ip.is_global:
            return False
    return True
