from math import ceil
from urllib.parse import urlencode

from sqlalchemy import delete
from sqlalchemy.orm import joinedload

from outception.auth.models import AuthSubject
from outception.config import settings
from outception.email.schemas import EmailUpdateEmail, EmailUpdateProps
from outception.email.sender import enqueue_email_template
from outception.exceptions import OutceptionError, OutceptionRequestValidationError
from outception.kit.crypto import generate_token_hash_pair, get_token_hash
from outception.kit.extensions.sqlalchemy import sql
from outception.kit.services import ResourceServiceReader
from outception.kit.utils import utc_now
from outception.models import EmailVerification
from outception.models.user import User
from outception.postgres import AsyncSession
from outception.user.repository import UserRepository

TOKEN_PREFIX = "outception_ev_"


class EmailUpdateError(OutceptionError): ...


class InvalidEmailUpdate(EmailUpdateError):
    def __init__(self) -> None:
        super().__init__(
            "This email update request is invalid or has expired.", status_code=401
        )


class EmailUpdateService(ResourceServiceReader[EmailVerification]):
    async def request_email_update(
        self,
        email: str,
        session: AsyncSession,
        auth_subject: AuthSubject[User],
    ) -> tuple[EmailVerification, str]:
        user = auth_subject.subject

        user_repository = UserRepository.from_session(session)
        existing_user = await user_repository.get_by_email(email)
        if existing_user is not None and existing_user.id != user.id:
            raise OutceptionRequestValidationError(
                [
                    {
                        "type": "value_error",
                        "loc": ("body", "email"),
                        "msg": "Another user is already using this email.",
                        "input": email,
                    }
                ]
            )

        token, token_hash = generate_token_hash_pair(
            secret=settings.SECRET, prefix=TOKEN_PREFIX
        )
        email_update_record = EmailVerification(
            email=email, token_hash=token_hash, user=user
        )

        session.add(email_update_record)
        await session.flush()

        return email_update_record, token

    async def send_email(
        self,
        email_update_record: EmailVerification,
        token: str,
        base_url: str,
        *,
        extra_url_params: dict[str, str] = {},
    ) -> None:
        delta = email_update_record.expires_at - utc_now()
        # total_seconds(), not .seconds - the latter drops whole days, so a TTL
        # over a day would under-report the lifetime in the email.
        token_lifetime_minutes = int(ceil(delta.total_seconds() / 60))

        email = email_update_record.email
        url_params = {"token": token, **extra_url_params}
        enqueue_email_template(
            EmailUpdateEmail(
                props=EmailUpdateProps(
                    email=email,
                    token_lifetime_minutes=token_lifetime_minutes,
                    url=f"{base_url}?{urlencode(url_params)}",
                )
            ),
            to_email_addr=email,
            subject="Update your email",
        )

    async def verify(self, session: AsyncSession, token: str, user: User) -> User:
        token_hash = get_token_hash(token, secret=settings.SECRET)
        email_update_record = await self._get_email_update_record_by_token_hash(
            session, token_hash
        )

        if email_update_record is None or email_update_record.user_id != user.id:
            raise InvalidEmailUpdate()

        # Re-checked at verify, not only at request: the address can be
        # registered by someone else inside the token's 30-minute window,
        # and the unique index would turn this into an unhandled 500.
        user_repository = UserRepository.from_session(session)
        # included_blocked: a blocked account keeps its real address and the
        # unique index covers ALL rows, so excluding blocked users here let
        # the exact 500 this check exists to stop through. A registration
        # landing between this read and the commit can still hit the index -
        # rare enough to accept, and the index keeps the data correct.
        existing_user = await user_repository.get_by_email(
            email_update_record.email, included_blocked=True
        )
        if existing_user is not None and existing_user.id != user.id:
            raise InvalidEmailUpdate()

        user = email_update_record.user
        user.email = email_update_record.email
        session.add(user)

        await session.delete(email_update_record)

        return user

    async def _get_email_update_record_by_token_hash(
        self, session: AsyncSession, token_hash: str
    ) -> EmailVerification | None:
        statement = (
            sql.select(EmailVerification)
            .where(
                EmailVerification.token_hash == token_hash,
                EmailVerification.expires_at > utc_now(),
            )
            .options(joinedload(EmailVerification.user))
        )

        res = await session.execute(statement)
        return res.scalars().unique().one_or_none()

    async def delete_expired_record(self, session: AsyncSession) -> None:
        statement = delete(EmailVerification).where(
            EmailVerification.expires_at < utc_now()
        )
        await session.execute(statement)
        await session.flush()


email_update = EmailUpdateService(EmailVerification)
