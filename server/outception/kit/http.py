...
async def check_url_reachable(
    ...
    try:
        async with httpx.AsyncClient(
            ...
            timeout=timeout,
            headers={"User-Agent": settings.OUTCEPTION_USER_AGENT},
...
def get_safe_return_url(return_to: str | None) -> str:
    ...
    if not url_info.netloc:
        ...
        # "https://outception.com@evil.com", which browsers interpret as navigating
...
def _peer_is_trusted_proxy(peer: str) -> bool:
    """True if the connecting peer falls inside a configured trusted-proxy
    range. Only then do we honour client-IP forwarding headers."""
...
def get_ip_address(request: Request) -> str | None:
    ...
    Honour a client-IP forwarding header ONLY when the request's immediate peer
    is a configured trusted proxy (`TRUSTED_PROXY_IPS`); otherwise the header is
    attacker-controllable and is ignored in favour of the socket peer IP.
    Trusting it unconditionally would let anyone reset their rate-limit bucket /
    evade IP bans by rotating the header.

    The accepted headers come from `TRUSTED_CLIENT_IP_HEADERS` (first present
    wins). Behind the documented Cloudflare -> Caddy edge the real client IP
    arrives in `CF-Connecting-IP` (all plans); `True-Client-IP` is Enterprise
    only, so honouring only that header would collapse every request to the
    Caddy container's IP.
    """
    peer = request.client.host if request.client else None

    if peer is not None and _peer_is_trusted_proxy(peer):
        for header in settings.TRUSTED_CLIENT_IP_HEADERS:
            forwarded = request.headers.get(header)
            if forwarded:
                # CF-Connecting-IP / True-Client-IP are single IPs; guard
                # against a comma-list leaking in and take the first entry.
