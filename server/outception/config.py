...
class Settings(BaseSettings):
    ...
    # exists for. Sized above the observed day-one spend (1,462 — which
    # included the one-off 330-source backfill) so ordinary churn never
    # brushes it: pre-tap sweeps ~330 sources x 3 heroes as they rotate, plus
    # the 6-deep candidates from viewed cards. Worst case all of it lands on
    # Gemini: 2,500 warm + the readers' half of the total cap still sits
    # under half the fleet's ~10k/day free capacity (9 Gemini keys + Groq +
    # Mistral), and the warmer never touches paid regardless.
    SUMMARY_WARM_DAILY_CAP: int = 2500
