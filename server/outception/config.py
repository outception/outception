...
class Settings(BaseSettings):
    ...
    # NVIDIA NIM (build.nvidia.com, Developer Program, no card): 40 requests a
    # minute, daily allowance undocumented. Kimi K3 was the strongest model
    # the account could reach when probed 2026-09-09 (gpt-oss-120b retired
    # there 2026-09-03; Llama/Nemotron/Gemma answered 404 for the account,
    # DeepSeek timed out): excellent Croatian and Slovenian, but 5-7 s per
    # batch on the shared free queue - too slow for a reader's first paint,
    # right for the background lanes (warmer, translate-ahead), which is
    # where it frees Gemini and Groq for readers.
    NVIDIA_API_KEY: str | None = None
    NVIDIA_API_KEYS: str | None = None
    NVIDIA_MODEL: str = "moonshotai/kimi-k3"
    NVIDIA_RPM_CAP: int = 30
    # Ollama Cloud (ollama.com, free tier, no card): the same gpt-oss-120b
    # Groq serves, about 1 s a batch when probed 2026-09-09 - a reader-speed
    # lane with its own quota. The free tier is metered by GPU time in 5-hour
    # and weekly windows rather than published request counts, so the cap is
    # conservative and the usual benching absorbs a 429 when a window is spent.
    OLLAMA_API_KEY: str | None = None
    OLLAMA_API_KEYS: str | None = None
    OLLAMA_MODEL: str = "gpt-oss:120b"
    OLLAMA_RPM_CAP: int = 20
    # Cloudflare Workers AI (every account gets 10,000 neurons a day free,
    # a few hundred batches; the account already exists for the deploy). The
    # endpoint is per account, so the id rides along with the token (a
    # Workers AI token cannot list accounts to discover it). Probed
    # 2026-09-09 with the server's request body: gpt-oss-120b answers in
    # ~3 s with good Croatian and Slovenian - the same model Groq and Ollama
    # serve, and cheaper in neurons than Llama 3.3 70B for output-heavy
    # translation (its reply time was the same).
    ...
    # Our own ceiling on Cloudflare requests per UTC day, well inside the
    # 10,000 free neurons (a translation batch is ~40 neurons on gpt-oss-120b,
    # a summary several times that), so the account never reaches the point
    # where a Workers Paid plan would start billing. Enforced in free_llm.
    ...
    # Of that warm allowance, how much may go on SPECULATIVE warming - the
    # roster sweep summarising articles nobody has opened yet. Measured in
    # production 2026-09-10: 1,192 of the day's 1,216 summaries were this,
    # and because summaries and headline translation share the same Gemini
    # fleet, that speculation was the fleet's largest single consumer while
    # cold cards waited on the slow translation lanes. Warming for cards
    # readers actually opened is NOT capped by this - only the guesswork.
    ...
    # a reader's card set warm in the languages actually being read.
    ...
    # The warmer's daily share of Gemini's FREE quota (batches).
    #
    # Raised in steps from 600 as production measurement showed the warmer
    # stopping on our own numbers rather than on anything real: 14 keys sat
    # unbenched, the run hit its batch cap with time to spare, and only one
    # language qualified. 14,000 is deliberately ABOVE what the warmer can
    # physically spend in a day, so this stops being the binding limit at all.
    #
    # What bounds it instead, and what to change if it needs reining in:
    #   _WARM_LANGUAGE_CAP     8 languages, whatever the demand ranking says
    #   _WARM_DEMAND_SOURCE_CAP  40 recently-read sources on top of the defaults
    #   _WARM_RUN_BATCH_CAP    400 batches per ten-minute tick
    #   _WARM_CONCURRENCY      3, so warming never bursts against live readers
    # A full cold sweep of 8 languages across ~78 sources is roughly 1,250
    # batches; after that the warmer only translates headlines that appeared
    # since, so normal days sit far below this number.
    #
    # The risk this DOES accept: the warmer and live readers drink from the
    # same free pool, and the old small cap was what reserved the day's
    # remainder for readers. The signal that it has gone too far is Gemini keys
    # getting benched (news:gemini:cd:*) and the provider mix shifting toward
    # paid. Watch those rather than this number.
    #
    # FREE only. The paid allowance below is untouched, so if the free tier
    # does run dry the warmer stands down instead of reaching for the card.
    TRANSLATION_WARMER_FREE_DAILY_CAP: int = 14000
    ...
    # Background cache-warmer: on a schedule, pre-translate the default cards'
    ...
    LOGFIRE_IGNORED_ACTORS: set[str] = {
        ...
        # props carry the live login code and the email-update URL with its
        # raw token - email_logs redacts them at rest, but the span fired
        # first. Third-party log stores must never hold live credentials.
        "email.send",
    }
    # S3 logs storage
