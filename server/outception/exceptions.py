...
class OutceptionError(Exception):
    """
    Base exception class for all errors raised by Outception.
    ...
    # 5xx responses that are a documented outcome of the endpoint rather than
    # a fault (see `outception.sentry.before_send`).
...
class OutceptionBackpressureError(OutceptionError):
    ...
    client's contract is "retry shortly", and Sentry drops these (see
    outception.sentry.before_send); readers hitting a cold heatmap after a
    deploy paged as a regression 61 times before this subclass existed."""
...
class OutceptionRedirectionError(OutceptionError):
    """
...
class Unauthorized(OutceptionError):
...
class OutceptionRequestValidationError(OutceptionError):
