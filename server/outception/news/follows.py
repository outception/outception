...
from .schemas import NewsItem, NewsSearchItem
...
async def followed_feed(
    ...
    #
    # Rank on the raw dicts, validate only the winners: model_validate over
    # every item of a maxed 500-follow set was ~15,000 model constructions of
    # event-loop CPU per request to return 40 rows.
        ...
        for source_id, raw in await cache.mget_hot_raw(redis, chunk):
            ...
            for item in items:
                if isinstance(item, dict):
                    ...
                    rows.append((pub if isinstance(pub, int) else 0, source_id, item))
    rows.sort(key=lambda row: row[0], reverse=True)
    hits: list[NewsSearchItem] = []
    for _, source_id, item in rows:
        if len(hits) >= limit:
            break
        try:
            parsed = NewsItem.model_validate(item)
