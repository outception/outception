...
DISABLED_SOURCES: frozenset[str] = frozenset(
    {
        # 2026-09-02 audit: the feed still answers 200, but the publisher
        # stopped posting to it - every item is months to years old. Their
        # Google News listing is equally cold, so there is nothing to
        # repoint to (the ones that were still publishing were repointed).
        "dlnews",
        "imore",
        "joelonsoftware",
        "legaldive",
        ...
        "tuko",
        "walesonline",
        # 2026-08-31 audit, same criterion.
        "autojosh",
        # Pre-audit disables.
        "9news_au",
        "9to5linux",
...
def resolve_known(source_id: str) -> str | None:
    """Follow a redirect alias to the canonical catalogue id, getter or not:
    heatmap and game cards have no getter (their tiles come from their own
    endpoints) but are followable like any card. None when the id is unknown
    or disabled."""
    meta = SOURCES.get(source_id)
    if meta is None:
        return None
    source_id = meta.get("redirect") or source_id
