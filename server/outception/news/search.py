...
async def search_headlines(
    ...
    # Collect the candidate source ids first, then read them all in one MGET
    # instead of a GET-per-key. Otherwise a warm cache turns a single search
    # into hundreds of sequential Redis round trips + JSON parses.
    source_ids: list[str] = []
    scanned = 0
    async for raw_key in redis.scan_iter(match="news:source:*", count=200):
        ...
        source_ids.append(key.rsplit(":", 1)[-1])

    # One MGET fetches every candidate payload (vs a GET per key), but parse
    # lazily so filling `limit` from the first few sources doesn't force
    # deserializing all of them.
    for source_id, raw in await cache.mget_hot_raw(redis, source_ids):
