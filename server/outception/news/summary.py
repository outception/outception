...
from . import free_llm, gemini, gnews
...
# Whole-stream bound for the streaming (web) path: time to the first token
# plus the same generation budget the non-streaming path gets. Without it only
# the FIRST chunk was deadlined and the body could run as long as the model
# trickled - httpx's timeout is per read, not per stream - so the leader had no
# ceiling at all and a waiter below could time out while it was still writing.
# Measured on the live API a cold summary completes end to end in ~6 s, so this
# is roughly double the real cost and should fire only on a stalling provider.
_LIVE_STREAM_SECONDS = _LIVE_FIRST_CHUNK_SECONDS + _LIVE_MODEL_SECONDS
...
# so raising either deadline can't silently reintroduce that race. The leader
# may be on either path, so take the slower: deriving this from the
# non-streaming budget alone was the race again, because the streaming leader
# outlives it.
_PENDING_WAIT_SECONDS = (
    _LIVE_ARTICLE_SECONDS + max(_LIVE_MODEL_SECONDS, _LIVE_STREAM_SECONDS) + 2.0
)
# The marker must outlive the wait or it expires under a waiter still watching
# it, which is the single-flight collapsing into duplicate paid generations.
# Asserted rather than commented: this file derives the wait from four other
# deadlines, and raising any of them silently broke the relationship before.
assert _PENDING_WAIT_SECONDS < _PENDING_TTL_SECONDS
...
_TEASER_JUNK = re.compile(
    ...
    # The CDN 403 bodies, which say none of the above. Akamai's is the one
    # that reached a card: "You don't have permission to access <url> on this
    # server. Reference #18.e3dd717... https://errors.edgesuite.net/...".
    # Shown as the publisher's own summary, so it cost
    # model budget to print an error page in Croatian.
    # "...to access/view" only: a council story about residents who "do not
    # have permission to park" is ordinary prose, not a blocked fetch.
...
# One article legitimately gets summarized in a handful of languages a day;
# every known URL x 48 canonical languages is a distinct digest and a fresh
# model call, which made the whole daily budget anonymously drainable by
# cycling ?lang= over public URLs. Counted only when a generation is really
# about to run (cache hits never touch it).
_URL_FANOUT_KEY = "news:summary:urlday:{digest}:{day}"
_URL_FANOUT_DAILY_CAP = 8


async def _url_fanout_exceeded(redis: Redis, url: str) -> bool:
    key = _URL_FANOUT_KEY.format(
        digest=_digest(url, "urlday"), day=datetime.now(UTC).strftime("%Y%m%d")
    )
    count = int(await redis.incr(key))
    if count == 1:
        await redis.expire(key, 2 * 24 * 60 * 60)
    if count > _URL_FANOUT_DAILY_CAP:
...
async def _charge_daily(
    ...
    two counters move in lockstep: the reader-spend brake subtracts warm from
    total, and any charge that lands in one but not the other makes the brake
    lie (warm failures once counted as reader traffic that never happened).
    ``pretap`` additionally charges the speculative sub-budget - warming for
    articles nobody has opened, which shares the Gemini fleet with headline
    translation and so must not be able to spend the whole allowance."""
    day = datetime.now(UTC).strftime("%Y%m%d")
    if await redis.incr(_DAILY_KEY.format(day=day)) == 1:
        await redis.expire(_DAILY_KEY.format(day=day), 2 * 24 * 60 * 60)
    if warm:
        if await redis.incr(_WARM_DAILY_KEY.format(day=day)) == 1:
            await redis.expire(_WARM_DAILY_KEY.format(day=day), 2 * 24 * 60 * 60)
    if pretap:
        if await redis.incr(_PRETAP_DAILY_KEY.format(day=day)) == 1:
            await redis.expire(_PRETAP_DAILY_KEY.format(day=day), 2 * 24 * 60 * 60)
...
# Scripts a summary must never slip into. English output is Latin, so anything
# from these blocks is the model leaving the language mid-sentence - except
# where the scraped page carried the same characters (a name in its own
# script), which is excused token by token below.
...
def _script_mismatch(text: str, source: str = "") -> bool:
    """True when *text* carries non-Latin characters the scraped page did not.

    A foreign character fused straight into a Latin word ("jasne政策 okvire")
    is the model slipping mid-word, but brand names genuinely fuse scripts
    ("小米SU7", "5μm"), so a fused token copied verbatim out of the source is
    left alone. The source here is a scraped page whose nav and footer junk
    carries stray foreign characters, so a bare presence match is not enough
    on its own - only the fused check is decisive."""
...
def _system_prompt(lang: str) -> str:
    return (
        ...
        " message, or an error page). Respond in English, using only the"
        " Latin script: never mix scripts or slip a character from another"
        " writing system into a word."
    )
...
async def _summarize_stream(redis: Redis, text: str, lang: str) -> AsyncGenerator[str]:
    ...
    # Reserve BEFORE calling: read-then-count let N concurrent requests all
    # see "under cap" and overshoot by the in-flight concurrency - which an
    # attacker saturating the free tiers controls. INCR is atomic; a failed
    # call refunds its slot.
    ...
    if used > settings.SUMMARY_PAID_DAILY_CAP:
        ...
        if used == settings.SUMMARY_PAID_DAILY_CAP + 1:
            log.warning("news.summary_paid_cap", used=used - 1)
...
async def _summarize(redis: Redis, text: str, lang: str) -> str:
    ...
    if settings.ANTHROPIC_API_KEY is None:
        raise ValueError("no summary provider available")
    # Reserve BEFORE calling (see _summarize_stream): read-then-count raced
    # under concurrency and overshot the paid cap.
    day_key = _PAID_DAILY_KEY.format(day=datetime.now(UTC).strftime("%Y%m%d"))
    used = int(await redis.incr(day_key))
    if used == 1:
        await redis.expire(day_key, 2 * 24 * 60 * 60)
    if used > settings.SUMMARY_PAID_DAILY_CAP:
        ...
        raise ValueError("paid summary cap reached")
    try:
        return await _summarize_anthropic(text, lang)
    except BaseException:
        await redis.decr(day_key)
        raise
...
async def _started_in_time(
    stream: AsyncGenerator[str], seconds: float, total: float
) -> AsyncIterator[str]:
    ...
    attention, one that has not is given up on before the panel does.

    The total matters as much as the first chunk. httpx's timeout applies per
    read, not per stream, so a model trickling one token at a time had no
    ceiling at all - and the duplicate-tap wait below is derived from that
    ceiling, so an unbounded leader meant a waiter timing out while the
    summary it wanted was still being written."""
...
async def _produce(
    ...
    *,
    ...
    # gemini_only is the warmer's mode: its charges belong to the warm
    # sub-budget too (see _charge_daily on why the lockstep matters).
    await _charge_daily(redis, warm=gemini_only, pretap=pretap)
    ...
    if _script_mismatch(summary, text):
        ...
        if not summary or _script_mismatch(summary, text):
            raise ValueError("mixed script in summary")
...
async def _teaser_result(
    ...
    description first, else what the page exposed. No summarizer."""
    ...
    if not teaser:
        return None
    # The publisher's own line, as published. No model call at all: these run
    # exactly on the paths that exist because the summary budget is spent or
    # the article failed, so they must never be the thing that spends more.
    await redis.set(
...
def _stream_result(result: SummaryResult) -> dict[str, object]:
    """One finished result as a stream event."""
...
async def stream_summary(
    ...
    cached = await _cached_result(redis, digest)
    if cached is not None:
        yield _stream_result(cached)
        return
    if await redis.get(_FAIL_KEY.format(digest=digest)) is not None or (
        await _host_blocked(redis, url)
    ):
        result = await _teaser_result(redis, url, lang, digest, None)
        yield (_stream_result(result) if result else {"error": "unavailable"})
        return
    pending_key = _PENDING_KEY.format(digest=digest)
    if not await redis.set(pending_key, "1", ex=_PENDING_TTL_SECONDS, nx=True):
        try:
...
async def _stream_generate(
    ...
    if used >= settings.SUMMARY_DAILY_CAP:
        ...
        yield (_stream_result(result) if result else {"error": "unavailable"})
        return
    if await _url_fanout_exceeded(redis, url):
        result = await _teaser_result(redis, url, lang, digest, None)
        yield (_stream_result(result) if result else {"error": "unavailable"})
        return
    ...
    try:
        ...
        async for chunk in _started_in_time(
            _summarize_stream(redis, text, lang),
            _LIVE_FIRST_CHUNK_SECONDS,
            _LIVE_STREAM_SECONDS,
        ):
            if held is not None:
                held += chunk
...
async def _cached_result(redis: Redis, digest: str) -> SummaryResult | None:
    ...
    if held is not None:
        if _TEASER_JUNK.search(held):
            # Poisoned before the junk filter existed:
...
# Speculative candidates from the buzz warmer's pre-tap pass: heroes of cards
# NOBODY has opened yet. A separate queue drained only after the viewed-card
# queue is empty - when they shared one set, a warmer rotation could SADD
# hundreds of speculative entries and randomly starve the heroes of cards
# readers were actually looking at.
_WARM_PRETAP_KEY = "news:summary:warm:pretap"
...
_WARM_KICK_KEY = "news:summary:warm:kick"
_WARM_QUEUE_TTL_SECONDS = 6 * 60 * 60
_WARM_QUEUE_MAX = 500
_WARM_URGENT_MAX = 200
_WARM_DAILY_KEY = "news:summary:warmday:{day}"
# Speculative ("pretap") warming is counted separately from warming for
# cards readers actually opened, so it can have a budget of its own.
_PRETAP_DAILY_KEY = "news:summary:pretapday:{day}"
...
async def note_warm_candidate(
    ...
    if urgent:
        ...
        # interval. NX-debounced (30s): a brownout turns every failed tap
        # into a handoff, and each kick was its own broker message. The run
        # lock is non-blocking, so a kick while a run is already draining
        # simply no-ops - and the job queue is request-bound (flushed by
        # middleware), absent in bare scripts/tests: missing manager just
        # means the cron picks the handoff up as before.
        if results[-1]:
            try:
...
async def note_warm_candidates(
    ...
    the whole batch, where per-URL calls cost two round trips EACH on the
    hottest read path (six per card serve). ``pretap`` routes speculative
    candidates (heroes nobody has opened) to their own second-class queue."""
    if not gemini.configured() and not free_llm.configured():
        return
    payloads = [
        f"{lang}\t{url}"
        for url in urls
        if url.startswith(("http://", "https://")) and not _is_unsummarizable(url)
    ]
    if not payloads:
        return
    queue_key = _WARM_PRETAP_KEY if pretap else _WARM_QUEUE_KEY
    if await redis.scard(queue_key) >= _WARM_QUEUE_MAX:
        return
    pipe = redis.pipeline()
    pipe.sadd(queue_key, *payloads)
    pipe.expire(queue_key, _WARM_QUEUE_TTL_SECONDS)
    await pipe.execute()


_WARM_ORIGIN_KEYS = {
    "urgent": _WARM_URGENT_KEY,
    "viewed": _WARM_QUEUE_KEY,
    "pretap": _WARM_PRETAP_KEY,
}
...
async def requeue_warm_candidate(
    ...
    """Put a popped candidate back where it came from (no kick, no trimming
    another entry out): the warmer draws it again when providers recover.
    RPUSH for the urgent FIFO - pop reads from the right, so this really is
    "back on top", not a rotation to the tail - and the TTL is refreshed so a
    queue kept alive only by requeues can't expire mid-brownout."""
    key = _WARM_ORIGIN_KEYS.get(origin, _WARM_QUEUE_KEY)
    pipe = redis.pipeline()
    if origin == "urgent":
        pipe.rpush(key, f"{lang}\t{url}")
    else:
        pipe.sadd(key, f"{lang}\t{url}")
    pipe.expire(key, _WARM_QUEUE_TTL_SECONDS)
    await pipe.execute()


async def pop_warm_candidate(redis: Redis) -> tuple[str, str, str] | None:
    """Next (url, lang, origin) from the warm queues, or None when they're
    drained. Live-tap handoffs first (oldest first), then heroes of viewed
    cards, and the speculative pre-tap queue only when both are empty.
    Malformed entries (legacy formats from before a deploy) are consumed and
    skipped rather than ending the whole run as a false "drained"."""
    for _ in range(50):
        origin = "urgent"
        ...
        if raw is None:
            origin = "pretap"
            raw = await redis.spop(_WARM_PRETAP_KEY)
        if raw is None:
            return None
...
async def warm_summary(
    ...
    'skipped' (already handled / budget exhausted), 'unavailable' (every free
    provider is benched - the candidate is still good, the task requeues it
    and stops draining) or 'failed' (generation error - the task's circuit
    breaker counts these)."""
    ...
    if not gemini_usable and not free_usable:
        return "unavailable"
    if lang != "en":
        # Queue entries predating the English-only cut must not spend the warm
        # budget on a language the reader can no longer ask for.
        return "skipped"
    digest = _digest(url, lang)
    ...
    #
    # These three brakes return "unavailable", not "skipped". "skipped"
    # DISCARDS the candidate - it has already been popped off the queue - and
    # skips the run's jitter, so a spent budget silently drained the whole
    # urgent FIFO, live taps' handoffs included, producing nothing. Nothing
    # more can be warmed this run either way, so put the candidate back and
    # stop the run (see the "unavailable" branch in tasks.warm_summaries).
    if total_used - warm_used > settings.SUMMARY_DAILY_CAP // 2:
        return "unavailable"
    # And regardless of who spent it: the warmer must never be the one that
    # walks the total into the global cap - the last fifth of the day's
    # budget belongs to live taps alone. (Without this, a quiet morning let
    # warm fill 3,500 of 5,000 before any reader showed up, and the
    # reader-spend brake above could then never fire.)
    if total_used >= settings.SUMMARY_DAILY_CAP * 4 // 5:
        return "unavailable"
    if warm_used >= settings.SUMMARY_WARM_DAILY_CAP:
        return "unavailable"
    # Speculative warming gets its own, much smaller allowance. It shares
    # the Gemini fleet with headline translation, and measured in
    # production it was taking ~1,190 of the day's 1,216 summaries for
    # articles nobody had opened, while cold cards waited on the slow
    # translation lanes. Warming for cards readers DID open is untouched.
    if origin == "pretap":
        pretap_used = int(await redis.get(_PRETAP_DAILY_KEY.format(day=day)) or 0)
    ...
    if slot is None and endpoint is None:
        # This minute's free calls belong to live readers - but that is a
        # transient condition, not "nothing to do". "skipped" DISCARDS the
        # candidate (it was already popped off the queue) and skips the run's
        # jitter, so a busy minute silently destroyed a live tap's handoff and
        # then ate the next candidates the same way. The reader's re-tap then
        # paid full cold generation again, which is exactly what the urgent
        # queue exists to prevent. "unavailable" puts it back and stops the
        # run, like any other benched-provider case.
        return "unavailable"

    try:
        # Bounded like every other background fetch (the 2026-08-14 incident
        # class): fetch_html's per-operation timeout compounds over retries,
        # redirects and the reader fallback, and one drip-feeding page could
        # otherwise eat minutes of the run between deadline checks.
