...
class SummaryResult:
    text: str
    kind: SummaryKind
    # True when `text` is a publisher teaser still in its ORIGINAL language:
    # the translation is running in the background and the next request
    # serves it. Such a result must not be cached anywhere (edge included)
    # and the client may poll for the translated line.
...
_TEASER_JUNK = re.compile(
    ...
    # Shown as the publisher's own summary, and translated first, so it cost
...
async def _teaser_result(
    ...
    if not teaser:
        return None
    # Cache-first, never a live model call (block=False): teasers run exactly
    # on the paths that exist because the summary budget is spent or the
    # article failed - a blocking translation here was a model call that
    # OUTLIVED the caps, buyable all day by cycling url x lang. A miss shows
    # the publisher's original line ONCE and stays uncached, so the next tap
    # picks up the background translation instead of a frozen original.
    translations, pending = await translate.translate_texts_with_status(
        redis, [teaser], lang, block=False
    )
    ...
    return SummaryResult(translated, "teaser", translation_pending=pending)
...
def _stream_result(result: SummaryResult) -> dict[str, object]:
    """One finished result as a stream event. `pending` marks a teaser still
    in its original language: the web has no second request to discover that
    (the /summary route reports it in the body), so without this flag the
    browser showed the publisher's English line and never asked again."""
    event: dict[str, object] = {"text": result.text, "kind": result.kind}
