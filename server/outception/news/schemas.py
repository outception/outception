...
class BatchRequest(BaseModel):
    ...
    # Headlines come back in this language where they are already cached.
    # Cache-only by design: this route never starts work (see the endpoint),
    # so anything not yet translated is returned as-is with
    # ``translationsPending`` set, and the card's own request fills it in.
...
class NewsSearchResponse(BaseModel):
    ...
    # True while some headlines are still being translated (see the card
    # endpoint): search and the followed feed are cache-first and never wait
    # on a model, so the first request in a language can only serve originals.
    translations_pending: bool = Field(False, alias="translationsPending")
...
class SummaryResponse(BaseModel):
    ...
    # A teaser still in the publisher's original language: its translation
    # is running in the background, request again shortly for the translated
    # line. Never true for a model-written summary.
    translations_pending: bool = Field(False, alias="translationsPending")
