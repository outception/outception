...
# Coordinates come straight from the caller, so each distinct rounded pair is
# an upstream call + a cache key. An ~11 km grid keeps forecasts right for
# readers while cutting the mintable key space 100-fold; the daily cap bounds
# what any traffic pattern can spend at Open-Meteo, whose quota ban would hit
# the egress IP - and every reader - at once. Real demand is nowhere near it:
# readers cluster in cities, and each grid cell is one upstream call per 15
# minutes at most.
_GRID_DECIMALS = 1
_UPSTREAM_DAILY_KEY = "news:weather:upstream:{day}"
_UPSTREAM_DAILY_CAP = 5000
# Distinct uncached locations one client may cause a day. A reader checks their
# own weather, maybe a second city; anything approaching this is a caller
# walking the coordinate grid, and the global cap above is what it would spend.
_CLIENT_DAILY_MISSES = 60
_FORECAST_DAYS = 4
...
def _shape(
    ...
    # Open-Meteo emits an explicit JSON null for a variable it can't supply, so
    # `.get(key, default)` returns None (the key IS there) and response
    # validation then 500s. Coalesce on the value, not on key presence.
    ...
    return {
        ...
        "current": {
            "temperature": _num(current.get("temperature_2m"), 0.0),
            "apparent_temperature": _num(current.get("apparent_temperature"), 0.0),
            "weather_code": _num(current.get("weather_code"), 0),
            "wind_speed": _num(current.get("wind_speed_10m"), 0.0),
            "humidity": _num(current.get("relative_humidity_2m"), 0),
            "is_day": bool(_num(current.get("is_day"), 1)),
        },
        "daily": [
            {
                "date": _text(days[i]),
                "weather_code": _num(_at(codes, i), 0),
                "temp_max": _num(_at(highs, i), 0.0),
                "temp_min": _num(_at(lows, i), 0.0),
            }
...
async def get_weather(
    ...
    key = _CACHE_KEY.format(
        lat=round(lat, _GRID_DECIMALS), lon=round(lon, _GRID_DECIMALS)
    )
    ...
    # Charged only on a cache miss, and only to the caller who caused it. The
    # grid is ~0.1 degrees, so a caller can mint millions of distinct cells;
    # the cache never repeats for them and the daily cap below is GLOBAL, so
    # one client walking coordinates could spend the whole day's upstream
    # allowance and leave every reader with a 502 until UTC midnight. A real
    # reader looks at one or two locations.
    if not await client_budget.spend(
        redis,
        "weather",
        client,
        limit=_CLIENT_DAILY_MISSES,
        window_seconds=24 * 60 * 60,
    ):
        raise OutceptionError("Weather is unavailable", status_code=502)

    day_key = _UPSTREAM_DAILY_KEY.format(day=datetime.now(UTC).strftime("%Y%m%d"))
    used = int(await redis.get(day_key) or 0)
    if used >= _UPSTREAM_DAILY_CAP:
        # The one-shot warning fires from the success branch below (the INCR
        # return is unique); warning here would log at request rate all day.
        raise OutceptionError("Weather is unavailable", status_code=502)
    ...
    # Charged on success only: failed upstream calls must not burn the cap
    # towards a self-inflicted daily blackout.
    count = await redis.incr(day_key)
    if count == 1:
        await redis.expire(day_key, 2 * 24 * 60 * 60)
    elif count == _UPSTREAM_DAILY_CAP:
        log.warning("news.weather_upstream_cap", used=count)
