...
async def faadelays() -> list[NewsItem]:
    ...
    # The FSM feed can carry more than one program for the same airport,

    # and ids are built from the airport code - dedupe so two rows can't

    # share an id (the card keys its expanded summary on it).
