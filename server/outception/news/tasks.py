"""Background translation warming for the default cards.
...
from . import cache, free_llm, gemini, heatmap, registry, summary, translate
...
# whole card set into every demanded language at once.
# Raised from 200 after the first run under the lower demand floor came back
# reporting exactly 200 warmed with `timed_out: false`, while 14 Gemini keys sat
# unbenched and the free allowance still had 400 spare. The run was being cut
# off by this number and nothing else, so cold-starting a newly qualified
# language took several ticks it did not need to.
_WARM_RUN_BATCH_CAP = 400
...
# Daily demand stamps a language needs before the warmer will spend budget on
# it.
#
# Measured on production rather than reasoned about. At a floor of 200 exactly
# ONE language qualified, while the warmer left 250 of its 600 free batches
# unspent every day:
#
#     hr 224   fr 109   sv 88   de 87   nl 64   sr 42   ko 38   rest single digits
#
# So the floor, not the budget, was what kept readers on the cold path, and it
# sat above real audiences rather than below them. 60 clears the natural gap
# under Dutch and admits the five languages people actually read, while the
# junk tail stays out. Lowered again to 35 once that run proved there was
# spare capacity behind it: that reaches Serbian (42) and Korean (38) too,
# and _WARM_LANGUAGE_CAP still stops it at eight however low this goes.
#
# This was never the real abuse guard and lowering it does not weaken one:
# _WARM_LANGUAGE_CAP is what bounds the damage, since demand is RANKED and only
# the top few are warmed however many codes someone stamps.
_WARM_DEMAND_FLOOR = 35
# Recently-viewed sources warmed alongside the default cards.
#
# Following is anonymous and device-local, so the server never sees a reader's
# card set. Recently-viewed sources are the closest thing it does see, and in
# practice they ARE the followed ones: the wall polls every visible card, so a
# reader's own sources stamp demand continuously while they read.
#
# Without this the warmer only ever covered the default set, so anyone who
# built their own cards, which is the whole point of the product, paid the full
# cold translation on every language switch.
#
# Deliberately NOT paired with a bigger batch cap. `uncached_count` skips a
# source whose headlines are already translated, so the per-run cap only ever
# counts real model work: once the defaults are warm they stop consuming it and
# the run reaches these instead. Coverage spreads across successive ticks
# rather than needing a larger budget up front, and the daily paid and free
# allowances stay the real ceiling.
_WARM_DEMAND_SOURCE_CAP = 40
...
def _card_sources() -> list[str]:
    return [
        sid
        for sid in DEFAULT_CARDS
        if sid != WEATHER_CARD_ID and sid in SOURCES and sid not in DISABLED_SOURCES
    ]


async def _demanded_sources(redis: Redis, exclude: set[str]) -> list[str]:
    """Sources readers opened in the demand window, most recently viewed first.

    Reads the set whole and sorts here, like the buzz warmer does: it is bounded
    by the 48 h window, and pruning is that warmer's job. Doing it here too
    would race it for no gain."""
    horizon = time.time() - SOURCE_DEMAND_TTL_SECONDS
    scored = await redis.zrange(SOURCE_DEMAND_KEY, 0, -1, withscores=True)
    out: list[str] = []
    for raw, score in sorted(scored, key=lambda pair: pair[1], reverse=True):
        if score < horizon:
            break
        source_id = raw.decode() if isinstance(raw, bytes) else raw
        if source_id in exclude or source_id == WEATHER_CARD_ID:
            continue
        if source_id not in SOURCES or source_id in DISABLED_SOURCES:
            continue
        out.append(source_id)
        if len(out) >= _WARM_DEMAND_SOURCE_CAP:
            break
    return out
...
async def warm_translations() -> None:
    """Pre-translate the default cards's cached headlines into the languages
    ...
    try:
        ...
        async with Locker(redis).lock(
            ...
            # Same floor the serve path uses (see endpoints._warm_languages).
            # Without it one caller cycling ?lang= codes stamps demand far
            # faster than real readers can and captures every warm slot, so
            # the languages people actually read get no pre-warming.
            languages = [
                lang
                for lang in await translate.demanded_targets(
                    redis, min_total=_WARM_DEMAND_FLOOR
                )
                if lang in allowed
            ][:_WARM_LANGUAGE_CAP]
            started = 0
            ...
            # Defaults first so a fresh reader is never made to wait behind
            # somebody else's card, then the sources readers actually opened.
            default_ids = _card_sources()
            walk = [
                *default_ids,
                *await _demanded_sources(redis, exclude=set(default_ids)),
            ]
            for source_id in walk:
            ...
            for language in languages:
                ...
                # Bounded by the SAME deadline that gates task creation.
                # Gathering without one meant the run budget only limited when
                # batches were STARTED, never how long the run waited on them:
                # a language's wave can take minutes on slow lanes, so the run
                # outlived its own Redis lock (budget + 60 s, never extended)
                # and the next tick started a second warmer against the same
                # sources and the same paid allowance.
                if not pending:
                    # Nothing cold left in this language: `uncached_count`
                    # skipped every source because they are all translated
                    # already. asyncio.wait() raises on an empty set, and this
                    # state used to be unreachable - with a single warmed
                    # language there was always cold work somewhere. Covering
                    # seven languages makes "everything is already warm" the
                    # NORMAL steady state, and it was aborting the whole run
                    # partway through, so the languages after it got nothing.
                    continue
                ...
                if unfinished:
                    ...
                    for task in unfinished:
                        task.cancel()
                    # Their per-chunk claims carry a short TTL, so a cancelled
                    # batch frees itself rather than blocking the next run.
...
async def warm_demanded_sources() -> None:
    ...
    try:
        async with Locker(redis).lock(
            ...
            for source_id in stale:
                ...
                try:
                    ...
                    # Translate the refreshed card into the languages its
                    # readers use while nobody is waiting on it, so their
                    # next open is warm (see translate.translate_ahead).
                    # Spawned, not awaited: the rotation keeps its 2-4 s
                    # cadence and the tasks are drained before the run ends.
                    translate.spawn_translate_ahead(
                        redis, source_id, [item.title for item in items]
                    )
        ...
        # Let the spawned translate-ahead batches finish on this connection
        # before it closes; bounded so a slow provider cannot hold the run
        # past the actor's time limit.
        await translate.drain_translate_ahead(timeout=60)
        await redis.close()
