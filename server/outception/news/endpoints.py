...
from .shopping_data import COUNTRY_DEALS, COUNTRY_PROPERTY, COUNTRY_TRAVEL
...
async def list_sources(response: Response) -> list[SourceMeta]:
    ...
    # origin, serving stale while it revalidates.
...
async def default_deck(
    ...
    deck: list[str] = []
    # Lead with the visitor's own national news + biggest-city local news when we
    # have feeds for them (news, sports and cities all localise by IP country).
    if cc:
        news_id = f"gnews-{cc.lower()}"
        if news_id in SOURCES and news_id not in DISABLED_SOURCES:
            deck.append(news_id)
        city_id = COUNTRY_TOP_CITY.get(cc)
        if city_id and city_id in SOURCES and city_id not in DISABLED_SOURCES:
            deck.append(city_id)
        # The visitor's own deals community and property-market card localise
        # the same way (their country's source, else nothing — never generic).
        # Kickstarter's launch feed is English-only, so it seeds only where
        # English is the primary reading language.
        for localized in (
            COUNTRY_DEALS.get(cc),
            COUNTRY_PROPERTY.get(cc),
            COUNTRY_TRAVEL.get(cc),
            "kickstarter" if cc in _ENGLISH_DECK else None,
        ):
            if localized and localized in SOURCES and localized not in DISABLED_SOURCES:
                deck.append(localized)
    injected = False
