...
async def batch(
    ...
    # Readers never touch the items, so they never need them as Python
    # objects: parsing and re-encoding ~2.9 MB reproduced bytes Redis had just
    # handed us (~10 ms in, ~7 ms out for a maxed request) on the route that
    # exists to make first paint instant. Splice the raw slice through
    # instead, and fall back to parsing if the payload is not the shape this
    # server writes.
