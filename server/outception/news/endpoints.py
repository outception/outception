    ...
    async def events() -> AsyncIterator[str]:
        ...
        try:
            ...
            if target is None or not (
                ...
                or await summary.has_cached(redis, url, target)
            ):
...
async def get_article_summary(
    ...
    # A teaser whose translation is still running is the publisher's ORIGINAL
    # line: served under the long public cache it sat at the edge for a day in
    # the wrong language for every reader of that url+lang (the app hits this
    # route; the web streams, which is never cached). Keep it uncacheable so
    # the next tap gets the translated line.
    response.headers["Cache-Control"] = (
        "no-store"
        if result.translation_pending
        else "public, max-age=3600, s-maxage=86400, stale-while-revalidate=604800"
    )
    response.headers["Vary"] = "Origin"
    return SummaryResponse(
        ...
        url=url,
        kind=result.kind,
        translations_pending=result.translation_pending,
    )
...
async def search_news(
    ...
    translated, pending = await _translate_hits(redis, items, lang)
    return NewsSearchResponse(
        sources=search.search_sources(q),
        items=translated,
        translations_pending=pending,
    )
...
async def followed_feed(
    ...
    translated, pending = await _translate_hits(redis, items, lang)
    return NewsSearchResponse(
        sources=[], items=translated, translations_pending=pending
    )
...
async def get_source(
    ...
    started = cache.now_ms()
    response = await _get_source(
        redis, source_id, latest=latest, client=get_ip_address(request)
    )
    originals = [item.title for item in response.items]
    # "success" also covers a fresh-enough cache hit; only an entry stamped
    # during this call was actually fetched now.
    fetched_now = response.status == "success" and response.updated_time >= started
    if lang and response.items:
        await translate.note_demand(redis, lang)
        await translate.note_source_language_demand(redis, response.id, lang)
        titles, pending = await translate.translate_texts_with_status(
            redis, originals, lang, block=False
        )
    ...
    if fetched_now and originals:
        # This request just refreshed the feed: translate the new headlines
        # into the languages this source's other readers use, now, in the
        # background - so their next open finds the card warm instead of
        # paying the model round trip at the soft wait.
        translate.spawn_translate_ahead(redis, response.id, originals, skip=lang)
    if response.items:
...
async def _translate_hits(
...
) -> tuple[list[NewsSearchItem], bool]:
    ...
    Also returns whether any title is still being translated. Like a card, the
    first request in a language can only serve originals while the batch runs;
    without this flag the client had no way to know to ask again, so a search
    made at that moment stayed in English until the reader retyped it."""
    lang = translate.canonical_target(lang) if lang else None
...
# `{"updated": <int>, "items": [...]}` - the exact shape cache.set writes.
# Anchored so it can only match the real keys, and it deliberately does NOT
# tolerate other shapes: a payload this does not recognise falls back to a
# normal parse rather than risking a wrong splice.
_PAYLOAD_RE = re.compile(r'^\{"updated":\s*(\d+),\s*"items":\s*(\[.*\])\}$', re.S)


def _split_payload(text: str) -> tuple[int, str] | None:
    """A cached payload as (updated, raw items JSON), without parsing the
    items. None when the payload is not the shape this server writes."""
...
async def batch(
    ...
    for source_id in body.sources:
        resolved = registry.resolve(source_id)
        if resolved is None or resolved in seen or resolved in DISABLED_SOURCES:
            continue
        seen.add(resolved)
    ...
    # Cache-only translation: this route must not start model work (a maxed
    # request covers 256 sources, so translating on demand here would be a
    # burst of thousands of headlines). Everything already translated is
    # served in the reader's language; the rest stays original and is flagged,
    # and the card's own ?lang= request translates it moments later. Without
    # this the wall painted English headlines and never corrected them.
    lang = translate.canonical_target(body.lang) if body.lang else None
    results: list[str] = []
    now = cache.now_ms()
    entries: list[tuple[str, int, list[dict[str, object]]]] = []
    # Untranslated readers never touch the items, so they never need them as
    # Python objects: parsing and re-encoding ~2.9 MB reproduced bytes Redis
    # had just handed us (~10 ms in, ~7 ms out for a maxed request) on the
    # route that exists to make first paint instant. Splice the raw slice
    # through instead, and fall back to parsing if the payload is not the
    # shape this server writes.
    ...
    for resolved, raw in await cache.mget_hot_raw(redis, resolved_ids):
        ...
        if not lang:
            ...
            spliced = _split_payload(text)
            if spliced is not None:
                raw_entries.append((resolved, spliced[0], spliced[1]))
                continue
        try:
            entry = json.loads(raw)
            entries.append((resolved, int(entry["updated"]), entry["items"]))
        except (ValueError, KeyError, TypeError):
            continue

    # One mget for every headline in the whole batch, not one per source.
    pending_ids: set[str] = set()
    if lang:
        flat: list[str] = []
        spans: list[tuple[str, int, int]] = []
        for resolved, _, items in entries:
            start = len(flat)
            flat.extend(str(item.get("title", "")) for item in items)
            spans.append((resolved, start, len(flat)))
        translated, missing = await translate.cached_translations_pending(
            redis, flat, lang
        )
        # Which headlines are still uncached comes from the cache read, never
        # from comparing the result to the original: a brand name or a headline
        # already in the reader's language translates to itself, so the
        # comparison marked fully warm cards pending for good and the wall
        # re-polled work that was already done.
        for (resolved, start, end), (_, _, items) in zip(spans, entries, strict=True):
            for offset, item in enumerate(items):
                item["title"] = translated[start + offset]
            # bisect, not a scan: a maxed request spans 256 sources and ~7,700
            # headlines, and `missing` is already ascending.
            if bisect_left(missing, start) < bisect_left(missing, end):
                pending_ids.add(resolved)

    for resolved, updated, items_json in raw_entries:
    ...
    for resolved, updated, items in entries:
        fresh = now - updated < registry.interval_ms(resolved)
        status = "success" if fresh else "cache"
        pending = "true" if resolved in pending_ids else "false"
        items_json = json.dumps(items, ensure_ascii=False)
        results.append(
            f'{{"status":"{status}","id":{json.dumps(resolved)},'
            f'"updatedTime":{updated},"items":{items_json},'
            f'"translationsPending":{pending}}}'
        )
    return Response(
