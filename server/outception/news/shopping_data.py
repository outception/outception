...
the default cards can lead with the visitor's own market (CF-IPCountry),
...
# The visitor country's travel-deals editor, swapped into the card set the same
# way (no generic fallback - the card set already carries a global travel card).
...
def fallback_search_feeds(
    ...
    neither a community feed nor a hand-localised search, so the seeded cards
...
def events_search_feeds(
    ...
    the cards seeding sees the picks."""
    out: list[ShoppingSearch] = []
    for cc, _hl, cname in countries:
...
def country_card_feeds(
    ...
    country edition. Only Business joins the seeded cards (COUNTRY_BUSINESS);
