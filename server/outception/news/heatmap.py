...
async def _refresh_in_background(redis: Redis, heatmap_id: str) -> None:
    ...
    tiles land in cache for the next reader."""
    try:
        await _fetch_and_store(redis, heatmap_id, cached=None)
    except OutceptionError:
        # Cold-failure bookkeeping already happened inside; the stale copy
        # keeps serving until the upstream recovers.
        pass
...
async def get_heatmap(redis: Redis, heatmap_id: str) -> dict[str, Any]:
    ...
    if cached is not None:
        ...
        if won:
            task = asyncio.create_task(_refresh_in_background(redis, heatmap_id))
