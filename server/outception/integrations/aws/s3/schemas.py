...
from outception.kit.schemas import IDSchema, Schema
...
class S3File(IDSchema, validate_assignment=True):
    ...
    def to_metadata(self) -> dict[str, str]:
        metadata = {
            "outception-id": str(self.id),
            "outception-organization-id": str(self.organization_id),
            "outception-name": self.name.encode("ascii", "ignore").decode("ascii"),
            "outception-size": str(self.size),
        }
        if self.checksum_sha256_base64:
            metadata["outception-coma256-base64"] = self.checksum_sha256_base64
        if self.checksum_sha256_hex:
            metadata["outception-coma256-hex"] = self.checksum_sha256_hex
        return metadata
    ...
    def from_head(cls, path: str, head: dict[str, Any]) -> Self:
        ...
        return cls(
            id=metadata.get("outception-id"),
            organization_id=metadata.get("outception-organization-id"),
            name=metadata.get("outception-name"),
            path=path,
            mime_type=head["ContentType"],
            size=metadata.get("outception-size"),
            storage_version=head.get("VersionId", None),
            checksum_etag=head.get("ETag", None),
            checksum_sha256_base64=metadata.get("outception-coma256-base64"),
            checksum_sha256_hex=metadata.get("outception-coma256-hex"),
