...
from outception.kit.http import ReturnTo, is_localhost
...
def _check_factor(
    ...
    for available_factor in available_factors:
        if available_factor.identifier == factor.identifier:
            return
    raise OutceptionAuthRedirectionError("Factor not available for this session")
    ...
    *,
    ...
    async def _authorize(
        ...
        try:
            ...
            raise OutceptionAuthRedirectionError(OIDC_ERROR_MESSAGE) from e
    ...
    async def _callback(
        ...
        if state is None:
            raise OutceptionAuthRedirectionError("Missing OAuth2 state")
        ...
        if request.method != "POST":
            ...
            if state_cookie is None:
                raise OutceptionAuthRedirectionError("Missing OAuth2 state cookie")

            if state != state_cookie:
                raise OutceptionAuthRedirectionError("Invalid OAuth2 state")
        ...
        try:
            ...
            raise OutceptionAuthRedirectionError("OAuth2 session expired") from e
