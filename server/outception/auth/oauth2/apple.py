...
from outception.models import OAuthAccount
...
class AppleFactor(OAuth2FactorMixin, AppleOAuth2FactorBase):
    ...
    async def get_email(
        ...
        # Same gate as Google: login links to an existing account by email,
        # so an UNVERIFIED address (possible on some managed Apple IDs) must
        # never authenticate as the account that owns it. Apple emits the
        # claim as a bool or the string "true".
