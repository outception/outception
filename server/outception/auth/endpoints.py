...
from outception.models import UserSession as UserSession
...
async def complete(
    ...
    # Every dashboard view is organization-scoped and there is no separate
    # onboarding step, so guarantee a personal organization at the
    # shared completion point of every login flow (email OTP, Google, Microsoft,
    # …). This also back-fills users created before org auto-provisioning.
    await user_service.ensure_personal_organization(session, user)
...
async def totp_enroll(
    ...
    return TOTPEnrollment(
        ...
        provisioning_uri=enrollment.get_provisioning_uri(user.email, "Outception"),
    )
