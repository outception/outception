...
pat_scheme = HTTPBearer(
    scheme_name="pat",
    auto_error=False,
    description="You can generate a **Personal Access Token** from your [settings](https://outception.com/settings).",
)
...
class _Authenticator:
    def __init__(
        ...
        *,
        ...
        # An empty set is almost certainly a mistake: `not self.required_scopes`
        # below treats it like None and silently disables the scope check.
        if required_scopes is not None and not required_scopes:
            raise ValueError("empty scopes disables the gate; pass None or real scopes")
