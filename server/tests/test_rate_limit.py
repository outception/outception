...
class TestAuthenticate:
    async def test_cache_miss_uses_client_ip_pending_auth(self, redis: Redis) -> None:
        identity = await _authenticate(
            _http_scope(
    ...
    async def test_rotating_unknown_tokens_share_one_bucket(self, redis: Redis) -> None:
        identities = {
            await _authenticate(
                _http_scope(
                    headers=[(b"authorization", f"Bearer outception_pat_{i}".encode())],
                    client=("8.8.8.8", 1234),
                ),
                redis=redis,
            )
        ...
        assert identities == {("8.8.8.8", RateLimitGroup.pending_auth)}
    ...
    async def test_cookie_cache_miss_uses_client_ip_pending_auth(
        self, redis: Redis
    ) -> None:
