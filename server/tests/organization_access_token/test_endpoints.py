...
from outception.models import Organization, OrganizationAccessToken, UserOrganization
...
async def _build_oat(
    ...
    *,
    ...
    token = OrganizationAccessToken(
        comment=comment,
        token=get_token_hash("outception_oat_test", secret=settings.SECRET),
        organization=organization,
...
class TestUpdateOrganizationAccessToken:
    ...
    async def test_oat_caller_cannot_elevate_scopes(
        ...
        response = await client.patch(
            ...
            json={"scopes": ["organizations:read"]},
        )
    ...
    async def test_oat_caller_within_scope_ok(
        ...
        existing: OrganizationAccessToken = await _build_oat(
            save_fixture,
            organization,
            scopes={Scope.organizations_read},
