...
def test_resolve_known_accepts_catalogue_ids_without_getter(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Heatmap/game cards: in the catalogue, no getter, still followable.
    monkeypatch.setitem(SOURCES, "phantom-source", {"name": "Phantom"})
    assert registry.resolve_known("phantom-source") == "phantom-source"
    assert registry.resolve_known("github") == "github-trending-today"
    assert registry.resolve_known("definitely-not-a-real-source") is None


def test_resolve_known_rejects_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    disabled = next(iter(registry.DISABLED_SOURCES & SOURCES.keys()))
    assert registry.resolve_known(disabled) is None
