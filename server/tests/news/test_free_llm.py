...
class TestEndpoints:
    ...
    def test_comma_separated_keys_fan_out(self, mocker: MockerFixture) -> None:
        ...
        assert {e.provider for e in pool} == {"groq", "mistral", "nvidia", "ollama"}
        nvidia, ollama = pool[-2], pool[-1]
        assert nvidia.url.startswith("https://integrate.api.nvidia.com/")
        assert nvidia.model == settings.NVIDIA_MODEL
        assert ollama.url == "https://ollama.com/v1/chat/completions"
        assert ollama.model == settings.OLLAMA_MODEL

    def test_cloudflare_needs_the_account_id(self, mocker: MockerFixture) -> None:
        ...
        (endpoint,) = free_llm.endpoints()
        assert endpoint.id == "cloudflare:0"
        assert endpoint.url == (
            "https://api.cloudflare.com/client/v4/accounts/abc123/ai/v1/chat/completions"
        )
        assert endpoint.model == settings.CLOUDFLARE_MODEL
...
class TestAcquire:
    ...
    async def test_provider_filter_narrows_the_pool(
        self, redis: Redis, mocker: MockerFixture
    ) -> None:
        _configure(mocker, GROQ_API_KEY="k1", MISTRAL_API_KEY="m1")
        for _ in range(3):
            endpoint = await free_llm.acquire(redis, {"mistral"})
            assert endpoint is not None
            assert endpoint.provider == "mistral"
        assert await free_llm.acquire(redis, {"nvidia"}) is None

    async def test_daily_cap_holds_cloudflare_inside_the_free_tier(
        self, redis: Redis, mocker: MockerFixture
    ) -> None:
        _configure(mocker, CLOUDFLARE_AI_TOKEN="cf1", CLOUDFLARE_ACCOUNT_ID="abc")
        mocker.patch.object(settings, "CLOUDFLARE_DAILY_CAP", 2)
        mocker.patch.object(settings, "CLOUDFLARE_RPM_CAP", 100)
        ...
        minute_keys = await redis.keys("news:freellm:rpm:cloudflare:0:*")
...
class TestCooldowns:
    ...
    def test_payment_required_is_a_dead_key(self) -> None:
        # A provider answers this on accounts with no free tier: retrying it
