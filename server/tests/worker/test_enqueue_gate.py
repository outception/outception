...
class TestFlushGate:
    async def test_disabled_routes_everything_to_redis(
        ...
        CorrelationID.set()
        jqm = JobQueueManager()
        jqm.enqueue_job("email.send", uuid4())
    ...
    async def test_allowlisted_actor_routes_to_sqs(
        ...
        customer_id = uuid4()
        jqm = JobQueueManager()
        jqm.enqueue_job("email.send", customer_id)  # HIGH priority, SQS
        jqm.enqueue_job("user.on_after_signup", uuid4())  # LOW priority, Redis
        await jqm.flush(dramatiq.get_broker(), redis)
        ...
        assert [job[0] for job in sent] == ["email.send"]
