...
You need an external URL that routes to your development server; [ngrok](https://ngrok.com/) works.
...
`dev docker` runs the whole stack in containers with hot-reloading. Use it for
isolated instances (parallel testing, several checkouts) or a setup that is
...
This builds the images, starts PostgreSQL, Redis and MinIO, installs
dependencies, runs migrations and starts the API, worker and web app.
...
```sh
dev docker up -d              # detached
dev docker logs [service]     # logs, optionally for one service
dev docker down               # stop everything
dev docker build              # rebuild images
dev docker shell api          # shell in a container
dev docker up --monitoring    # also start Prometheus + Grafana
```

### Multiple instances

`-i <n>` starts an isolated instance with its own containers, network, database,
Redis and MinIO. Ports are offset by `100 * n`.

```sh
dev docker up -d          # instance 0
dev docker up -i 1 -d     # instance 1
```
...
The API runs uvicorn with `--reload`, the worker runs dramatiq with `--watch`,
and the web app runs Next.js with Turbopack. Source changes on the host are
picked up by the containers.

### Configuration

Settings live in `dev/docker/.env.docker`, copied from
`dev/docker/.env.docker.template` on first run. Edit as needed.

The host-based workflow (`docker compose up -d` + `uv run task api`) is
