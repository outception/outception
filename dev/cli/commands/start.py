...
from shared import ROOT_DIR, SERVER_DIR, console, run_command, step_spinner
...
def _ensure_mouse_support() -> None:
    ...
    console.print(
        "  [dim]Mouse scrolling is not enabled in tmux. Enabling it lets you[/dim]"
    )
    ...
    if typer.confirm("  Enable mouse support in ~/.tmux.conf?", default=True):
        ...
        console.print(
            "  [dim]Tip: Ctrl-b then [ to scroll, q to exit scroll mode[/dim]\n"
        )
...
def register(app: typer.Typer, prompt_setup: callable) -> None:
    @app.command()
    def start() -> None:
        """Start all services (api, worker, web) in a tmux session."""
        if not prompt_setup():
            raise typer.Exit(1)

        if not _is_tmux_installed():
            if typer.confirm(
                "tmux is required but not installed. Install via Homebrew now?",
                default=True,
            ):
                ...
                if not result or result.returncode != 0 or not _is_tmux_installed():
                    console.print(
                        "[red]Installation failed. Install manually: brew install tmux[/red]"
                    )
                    raise typer.Exit(1)
            else:
                console.print(
                    "[yellow]tmux is required. Install it with: brew install tmux[/yellow]"
                )
                raise typer.Exit(1)
        ...
        if _session_exists():
            console.print(
                f"[bold blue]Attaching to existing '{SESSION}' session[/bold blue]\n"
            )
            os.execvp("tmux", ["tmux", "attach-session", "-t", SESSION])

        console.print(
            f"[bold blue]Starting Outception in tmux session '{SESSION}'[/bold blue]\n"
        )
