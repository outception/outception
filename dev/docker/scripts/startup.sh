...
if [[ "${1:-api}" == "api" && ! -f "$BOOTSTRAP_MARKER" ]]; then
    ...
    echo "Bootstrapping per-instance DB '$OUTCEPTION_POSTGRES_DATABASE' and MinIO buckets..."
...
s3 = boto3.client(
    "s3",
    endpoint_url=os.environ["OUTCEPTION_S3_ENDPOINT_URL"],
    aws_access_key_id=os.environ["OUTCEPTION_AWS_ACCESS_KEY_ID"],
    aws_secret_access_key=os.environ["OUTCEPTION_AWS_SECRET_ACCESS_KEY"],
    config=Config(signature_version="s3v4"),
    region_name="us-east-1",
)
private = os.environ["OUTCEPTION_S3_FILES_BUCKET_NAME"]
public = os.environ["OUTCEPTION_S3_FILES_PUBLIC_BUCKET_NAME"]
...
PY
    ...
    echo "Bootstrap complete: $OUTCEPTION_POSTGRES_DATABASE, $OUTCEPTION_S3_FILES_BUCKET_NAME, $OUTCEPTION_S3_FILES_PUBLIC_BUCKET_NAME"
fi
...
# may be stale.
if [[ -n "${OUTCEPTION_TINYBIRD_CLICKHOUSE_URL:-}" ]]; then
    TINYBIRD_HOST=$(echo "$OUTCEPTION_TINYBIRD_CLICKHOUSE_URL" | sed 's|http://||' | cut -d: -f1)
    ...
    if [[ -n "${TB_TOKEN:-}" ]]; then
        export OUTCEPTION_TINYBIRD_API_TOKEN="$TB_TOKEN"
        export OUTCEPTION_TINYBIRD_CLICKHOUSE_TOKEN="$TB_TOKEN"
        export OUTCEPTION_TINYBIRD_READ_TOKEN="$TB_TOKEN"
