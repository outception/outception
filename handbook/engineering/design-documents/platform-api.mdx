---
title: "Platform API"
description: "Proposal for a Platform API to enable partners to create and manage Outception Organizations on behalf of their users."
---
...
The Platform API enables partners to create and manage Outception Organizations on behalf of their users, allowing seamless integration of monetization features into third-party platforms. This document outlines the technical architecture, authentication flow, and claim process for the Platform API.
...
3. **Token Exchange**: JWT is sent to Outception's OAuth2 token endpoint
4. **Validation**: Outception validates the JWT using the platform's public key
5. **Access Token**: Outception returns an Organization Access Token
...
```json
{
  ...
  "aud": "outception:platform-organization", // Audience: organization operations
...
```json
{
  ...
  "aud": "outception:platform", // Audience: platform operations
  "iat": 1735689600, // Issued at timestamp
  "nbf": 1735689600, // Not before timestamp
  "exp": 1735776000 // Expiration timestamp
}
...
2. **Audience Check**: Ensure `aud` = `outception:platform-organization`
...
1. **JWT Validation**: Verify signature using platform's JWKS
2. **Audience Check**: Ensure `aud` = `outception:platform`
...
1. **Claim Token Generation**:
   ...
   - Outception generates a short-lived, single-use claim token
   - Token contains: `organization_id`, `platform_id`, expiration

2. **User Onboarding**:
   - Platform presents claim token to their user
   - User visits Outception's onboarding flow with claim token
   - User creates Outception account (or logs in)

3. **Organization Claiming**:
   - User submits claim token during onboarding
   - Outception validates token and links User to Organization
...
- `aud` is `outception:platform` (vs `outception:platform-organization` for org-specific operations)
...
- `POST /v1/oauth2/token` - Exchange JWT for access tokens
  - `grant_type: platform` with `aud: outception:platform` → Platform Access Token
  - `grant_type: platform` with `aud: outception:platform-organization` → Organization Access Token
...
- Standard Outception API endpoints (products, customers, subscriptions, etc.)

# Appendix: Sandbox as a Platform

Maybe a crazy idea, but we could consider the Outception Sandbox as a Platform on the Production environment. This would enable us to implement processes to automatically create an Organization in Production from a Sandbox!
