...
Introduce role-based access control for users on a Outception organization. Three roles ship in this iteration — `owner`, `admin`, and `member` — replacing today's implicit binary where org-level admin capability (member management, organization deletion, payout-account selection, Finance and payouts) is gated by `Account.admin_id`: the single user who is recorded as the admin on the org's Outception-side billing `Account`, today set at org creation.
...
Outception's authorization model has two distinct layers:
...
| Role | Permissions |
|---|---|
| `member` | Read and write across operational resources (products, customers, subscriptions, benefits, discounts, checkout links/sessions, orders, refunds, files, events, meters, custom fields, license keys, webhooks, metrics, notifications, customer seats) and read of the member list. **Excludes** Finance (`transactions:*`, `payouts:*`, `wallets:*`, `disputes:read`), member management (`members:invite`, `members:remove`, `members:set_role`), and org management (`organizations:edit_settings`, `organizations:delete`, `organizations:manage_payout_account`). OAT management (`organization_access_tokens:*`) is gated by `organization:manage` and therefore admin-only. Billing-info edits on the org's `Account` (legal name, address, etc.) fall under `organizations:edit_settings` and are therefore admin-only by way of the org-management exclusion. |
...
- **Notification routing and the Outception-for-Outception owner-side relationship** — re-anchored on the user holding `owner`. The user identified by these flows is unchanged; only the lookup path changes.
...
The recently-codified read-only-impersonation invariant ([#11303](https://github.com/outception/outception/pull/11303)) carries through: impersonation sessions hold no `_write` scope, and the role-mutation endpoint requires the `members:write` scope (which is what implies the `members:set_role` permission), so impersonation cannot mutate roles. No additional safeguards needed.
