...
2. Outception Checkout: Buyer enters email + card
3. Outception auto-creates individual customer
4. Post-purchase: Customer manages subscription via Customer Portal

#### Workflow B: Public Checkout (B2B - Current seats flow)
1. Customer visits merchant site, clicks checkout link
2. Outception Checkout: Buyer enters email + card
3. Outception auto-creates billing manager
...
2. Merchant creates Outception customer + business inside Outception via API
3. Billing manager clicks checkout in merchant app
4. Outception Checkout: Pre-filled information, enters card
...
5. **Outception Developer Experience** (weight: 3) - Avoid excessive filtering or complex queries
...
| Outception Dev Experience    | 🟢 3  | Clean queries, no type filtering, clear separation |
...
| Outception Dev Experience    | 🟡 2  | Requires type filtering, mitigated by SQLAlchemy polymorphism |
...
| Outception dev experience    | 3      | 🟢 3 (9)               | 🟡 2 (6)               |
...
We're introducing a new **Member** entity to improve how Outception handles team subscriptions and usage-based billing. This change provides:
...
| **Week 1-2** | One-on-one migration support | Work with Outception team if issues arise |
...
- Email support@outception.com with questions

**During migration**:
- Schedule 1-on-1 call if needed.

**After migration**:
- Report issues immediately: support@outception.com
- Check status page: status.outception.com
...
erDiagram
    ...
    Subscription {
        uuid id PK
        uuid customer_id FK "→ Customer (who pays)"
        uuid organization_id FK "→ Organization (denormalized from product)"
...
This appendix shows how merchants integrate with Outception for common workflows, comparing **Option 1 (Member Model)** vs **Option 2 (Customer Type + Member)** side-by-side.
...
```http
POST https://api.outception.com/v1/customers
Authorization: Bearer outception_secret_...
...
POST https://api.outception.com/v1/customers/cust_acme_123/members
Authorization: Bearer outception_secret_...
...
POST https://api.outception.com/v1/checkouts
Authorization: Bearer outception_secret_...
...
{
  "id": "checkout_abc",
  "url": "https://outception.com/checkout/abc",
  "customer_id": null  // Created after payment
}
```

```http
# Step 2: Customer completes checkout (Outception hosted)
# → Outception auto-creates Customer + Member
...
# Outception automatically creates BenefitGrant linked to ben_alice_789
```
...
```http
# Step 2: Customer completes checkout (Outception hosted)
# → Outception auto-creates Customer with type="individual"
...
# Outception automatically creates BenefitGrant linked to cust_alice_456
```
...
{
  "id": "checkout_def",
  "url": "https://outception.com/checkout/def",
  "customer_id": "cust_acme_123"
}
```
...
# → Outception auto-creates member for billing@acme.com
```
...
POST https://api.outception.com/v1/subscriptions/sub_acme_456/seats
Authorization: Bearer outception_customer_session_... // ben_billing_789 session
...
```http
# Step 5: Beneficiaries claim seats
POST https://api.outception.com/v1/subscriptions/sub_acme_789/seats
Authorization: Bearer outception_customer_session_... // cust_acme_456 session
...
https://buy.outception.com/outception_cl_gon2ISU4r7oYyjVaxXlRVUv5MKh1H3c1gCxKn1zZGHA
...
```http
# Merchant shares public checkout link
https://outception.com/org_abc/products/prod_team_plan

# Customer clicks, enters email, completes purchase
# → Outception detects seat-based product, auto-creates:
...
GET https://api.outception.com/v1/customer-portal/downloadables
Authorization: Bearer outception_customer_session_...
...
GET https://api.outception.com/v1/benefit-grants?member_id=ben_alice_acme_123
Authorization: Bearer outception_member_session_...
```
...
```http
# Download benefit
GET https://api.outception.com/v1/benefits/benefit_theme_download/download
Authorization: Bearer outception_customer_session_...
```
...
POST https://api.outception.com/v1/customer-sessions
Authorization: Bearer outception_secret_...
...
```json
{
  "token": "outception_cst_abc123xyz",
  "customer_id": "cust_bob_123",
  "member_id": "ben_bob_default_456",  // Auto-resolved from customer
  "return_url": "https://outception.com/org_merchant/portal?token=outception_cst_abc123xyz",
...
```json
{
  "token": "outception_cst_def456",
  "member_id": "ben_jane_789",
  "customer_id": "cust_acme_123",
  "url": "https://outception.com/portal?token=outception_cst_def456",
  "expires_at": "2025-12-01T10:00:00Z"
}
```
...
```http
# Option A: Using customer_id (B2C backward compatible)
POST https://api.outception.com/v1/events
Authorization: Bearer outception_secret_...
...
**What happens:**
- Outception checks if cust_alice_123 is a member of any businesses
...
The user access: https://outception.com/petru-test/portal/request
