...
2. In `polar-js/.speakeasy/workflow.yaml` change `https://api.outception.com/openapi.json` to `http://127.0.1:8000/openapi.json`
...
5. In the main outception repo, go to `clients/apps/web` and install the local dependency with `pnpm` e.g. `pnpm add "/Users/<user>/Development/polar-js"` (or whatever your local path to the `polar-js` repository is).
