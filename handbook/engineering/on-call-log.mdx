...
Running out of memory continuously on the workers, causing a crash loop after getting an influx of exceptions in the processing of them. [Read more discussion](https://outception-olw4768.slack.com/archives/C09T5UDPF2Q). Potentially related to the Sentry integration in Dramatiq being reenabled. We have now disabled it again which can be monitored if this continues to happen or if it stops now.


#### 2025-11-15

Webhooks were slow to process. Most likely due to an overload of other events causing a delay in processing. [Read more details](https://outception-olw4768.slack.com/archives/C093CJXJXPT/p1763221839002159).
...
- 20:18: [Number of processes/threads of the worker increased](https://github.com/outception/outception/commit/61e5f973ab46c55b1c11a9da744695b760884ad5) with no effect
- 20:32: [Patch deployed for task `customer_meter.update_customer` to skips retries and exits early if lock is occupied](https://github.com/outception/outception/commit/a67530c3d169920fdbad7fc9437bea94d88b9b96)
...
As an immediate fix for the issue: [we're updating our logic so that frequent `customer_meter.update_customer` calls are deduped](https://github.com/outception/outception/issues/7146).

We are also looking into another way to set up a [high-priority queue](https://github.com/outception/outception/issues/7149) now that we've learned that Dramatiq's built-in priority does nothing on Redis.
