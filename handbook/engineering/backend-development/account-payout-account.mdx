...
description: "How Outception's financial ledger entities work: Account (transaction ledger) and PayoutAccount (payout processor configuration)."
---

## Overview

Outception's financial architecture is split across two distinct entities:
...
erDiagram
    ...
    Transaction }o--o| Account : "account_id (nullable = Outception's own)"
...
- **Multiple organisations** can share a single `PayoutAccount` (e.g. a company with several Outception orgs pointing to one Stripe Connect account).
- **Accounts are created immediately** on org creation — no held balances, no pending state.

## `Account` Model

**`server/outception/models/account.py` · table `accounts`**
...
```python
currency: str              # Always "usd" (Outception's settlement currency)
...
`credit_balance` is **not** the merchant's earned revenue balance. It tracks **promotional fee credits** explicitly granted by Outception (manually by an admin, or via a campaign). Each credit is stored as an `AccountCredit` row; `credit_balance` is a denormalized running sum for fast reads.
...
**`server/outception/models/payout_account.py` · table `payout_accounts`**
...
**`server/outception/models/organization.py`**
...
**`server/outception/models/transaction.py` · table `transactions`**

```python
account_id: UUID | None    # NULL → Outception's own transaction; NOT NULL → merchant/seller
...
- `account_id IS NULL` → transaction is Outception's own (e.g. collected fees, outgoing transfers)

**Transaction types:** `payment`, `processor_fee`, `refund`, `refund_reversal`, `dispute`, `dispute_reversal`, `balance`, `payout`, `payout_reversal`

The future goal is to extend `Account` to represent customers and Outception subsidiaries too, enabling the full money trail to be traced in a single table:

```mermaid
flowchart LR
    C[Customer Account] -->|payment| P[Outception Account\nMerchant of Record]
...
**`server/outception/models/payout.py` · table `payouts`**
...
currency: str              # "usd" (Outception's side)
amount: int                # cents
...
status: PayoutStatus       # pending | in_transit | succeeded | failed | canceled | held
